...
class ALDM:
    ...
    def __init__(
        self, domain_information: DomainInformation, material_manager: MaterialManager
    ) -> None:
    ...
    def compute_fluxes_xi(
        self, prime: jnp.DeviceArray, cons: jnp.DeviceArray, axis: int, **kwargs
    ) -> jnp.DeviceArray:
    ...
    def solve_riemann_problem_xi(
        self,
        phi_L: jnp.DeviceArray,
        phi_R: jnp.DeviceArray,
        p3_L: jnp.DeviceArray,
        p3_R: jnp.DeviceArray,
        alpha_3: jnp.DeviceArray,
        fs: jnp.DeviceArray,
        axis: int,
    ) -> jnp.DeviceArray:
        ...
        R_diss = jnp.stack(
            [
                self._sigma_rho * jnp.abs(phi_delta[axis + 1])
                + fs * 0.5 * (jnp.abs(u_star) + jnp.abs(phi_delta[axis + 1])),
                self._sigma_rhou * jnp.abs(phi_delta[1])
                + fs * 0.5 * (jnp.abs(u_star) + jnp.abs(phi_delta[axis + 1])),
                self._sigma_rhou * jnp.abs(phi_delta[2])
                + fs * 0.5 * (jnp.abs(u_star) + jnp.abs(phi_delta[axis + 1])),
                self._sigma_rhou * jnp.abs(phi_delta[3])
                + fs * 0.5 * (jnp.abs(u_star) + jnp.abs(phi_delta[axis + 1])),
                self._sigma_rhoe * jnp.abs(phi_delta[axis + 1])
                + fs * 0.5 * (jnp.abs(u_star) + jnp.abs(phi_delta[axis + 1])),
            ]
        )
        ...
        flux_ui = [
            flux_rho * 0.5 * (phi_R[1] + phi_L[1])
            - R_diss[1] * 0.5 * (phi_R[0] + phi_L[0]) * (phi_R[1] - phi_L[1]),
            flux_rho * 0.5 * (phi_R[2] + phi_L[2])
            - R_diss[2] * 0.5 * (phi_R[0] + phi_L[0]) * (phi_R[2] - phi_L[2]),
            flux_rho * 0.5 * (phi_R[3] + phi_L[3])
            - R_diss[3] * 0.5 * (phi_R[0] + phi_L[0]) * (phi_R[3] - phi_L[3]),
        ]
        flux_rhoe = (
            u_star * 0.5 * (phi_R[5] + phi_L[5])
    ...
    def reconstruct_xi(
        self,
        phi: jnp.DeviceArray,
        alpha_1: jnp.DeviceArray,
        alpha_2: jnp.DeviceArray,
        alpha_3: jnp.DeviceArray,
        fs: jnp.DeviceArray,
        axis: int,
        j: int,
    ...
    def compute_phi(self, primes: jnp.DeviceArray, cons: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        :rtype: jnp.DeviceArray
        """
        rho_e = cons[4] - 0.5 * primes[0] * (
            primes[1] * primes[1] + primes[2] * primes[2] + primes[3] * primes[3]
        )
