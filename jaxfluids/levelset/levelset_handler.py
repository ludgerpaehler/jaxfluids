...
from jaxfluids.levelset.helper_functions import (move_source_to_target_ii,
                                                 move_source_to_target_ij,
                                                 move_source_to_target_ijk,
                                                 move_target_to_source_ii,
                                                 move_target_to_source_ij,
                                                 move_target_to_source_ijk)
from jaxfluids.levelset.interface_flux_computer import InterfaceFluxComputer
from jaxfluids.levelset.interface_quantity_computer import \
    InterfaceQuantityComputer
...
from jaxfluids.stencils import (DICT_DERIVATIVE_LEVELSET_ADVECTION,
...
class LevelsetHandler:
    ...
    def __init__(
        ...
        solid_interface_velocity: Union[List, types.LambdaType],
        boundary_condition: BoundaryCondition,
    ) -> None:
        self.geometry_calculator = GeometryCalculator(
            domain_information=domain_information,
            first_derivative_stencil=DICT_FIRST_DERIVATIVE_CENTER[
                numerical_setup["levelset"]["geometry_calculator_stencil"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
                offset=numerical_setup["levelset"]["halo_cells"],
            ),
            second_derivative_stencil=DICT_SECOND_DERIVATIVE_CENTER[
                numerical_setup["levelset"]["geometry_calculator_stencil"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
        ...
        self.extender_primes = QuantityExtender(
            ...
            time_integrator=DICT_TIME_INTEGRATION[
                numerical_setup["levelset"]["extension"]["time_integrator"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
            ),
            spatial_stencil=DICT_DERIVATIVE_QUANTITY_EXTENDER[
                numerical_setup["levelset"]["extension"]["spatial_stencil"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
            ),
        ...
        self.extender_interface = QuantityExtender(
            ...
            time_integrator=DICT_TIME_INTEGRATION[
                numerical_setup["levelset"]["extension"]["time_integrator"]
            ](nh=domain_information.nh_geometry, inactive_axis=domain_information.inactive_axis),
            spatial_stencil=DICT_DERIVATIVE_QUANTITY_EXTENDER[
                numerical_setup["levelset"]["extension"]["spatial_stencil"]
            ](nh=domain_information.nh_geometry, inactive_axis=domain_information.inactive_axis),
            is_interface=True,
        ...
        self.levelset_reinitializer = LevelsetReinitializer(
            ...
            boundary_condition=boundary_condition,
            time_integrator=DICT_TIME_INTEGRATION[
                numerical_setup["levelset"]["reinitialization"]["time_integrator"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
            ),
            derivative_stencil=DICT_DERIVATIVE_REINITIALIZATION[
                numerical_setup["levelset"]["reinitialization"]["spatial_stencil"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
            ),
        ...
        self.levelset_reinitializer_init = LevelsetReinitializer(
            ...
            boundary_condition=boundary_condition,
            time_integrator=DICT_TIME_INTEGRATION[
                numerical_setup["levelset"]["reinitialization"]["time_integrator_init"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
            ),
            derivative_stencil=DICT_DERIVATIVE_REINITIALIZATION[
                numerical_setup["levelset"]["reinitialization"]["spatial_stencil_init"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
            ),
        ...
        self.levelset_advection_stencil: SpatialDerivative = DICT_DERIVATIVE_LEVELSET_ADVECTION[
            numerical_setup["levelset"]["levelset_advection_stencil"]
        ](nh=domain_information.nh_conservatives, inactive_axis=domain_information.inactive_axis)
        ...
        self.mixing_targets = (
            numerical_setup["levelset"]["mixing_targets"]
            if "mixing_targets" in numerical_setup["levelset"].keys()
            else self.dim
        )
        ...
        self.steps_reinitialization_init = numerical_setup["levelset"]["reinitialization"][
            "steps_init"
        ]
    ...
    def compute_masks(
        self, levelset: jnp.DeviceArray, volume_fraction: jnp.DeviceArray
    ...
    def compute_volume_fraction_and_apertures(
        self, levelset: jnp.DeviceArray
    ) -> Tuple[jnp.DeviceArray, List]:
        ...
        """
        volume_fraction, apertures = self.geometry_calculator.linear_interface_reconstruction(
            levelset
        )
    ...
    def extend_primes(
        ...
        volume_fraction: jnp.DeviceArray,
        current_time: float,
        mask_small_cells: jnp.DeviceArray = None,
        ...
        normal = self.geometry_calculator.compute_normal(levelset)[
            ..., self.nhx_, self.nhy_, self.nhz_
        ]
        normal_extend = (
            jnp.stack([-normal, normal], axis=1) if self.levelset_type == "FLUID-FLUID" else -normal
        )
        mask_real, _ = self.compute_masks(levelset, volume_fraction)
        mask_ghost = 1 - mask_real[..., self.nhx_, self.nhy_, self.nhz_]
        mask_narrow_band = jnp.where(
            jnp.abs(levelset[self.nhx, self.nhy, self.nhz]) / self.smallest_cell_size
            < self.narrow_band_computations,
            1,
            0,
        )
        mask_extend = (
            jnp.maximum(mask_ghost * mask_narrow_band, mask_small_cells)
            if mask_small_cells != None
            else mask_ghost * mask_narrow_band
        )

        # EXTEND PRIMES
        primes, max_residual = self.extender_primes.extend(
            primes,
            normal_extend,
            mask_extend,
            self.CFL_extension_primes,
            self.steps_extension_primes,
        )

        # VELOCITY TREATMENT FOR SOLIDS
        if self.levelset_type in ["FLUID-SOLID-STATIC", "FLUID-SOLID-DYNAMIC"]:
            if self.is_viscous_flux:
                primes = primes.at[1:4, self.nhx, self.nhy, self.nhz].mul(
                    jnp.where(mask_ghost == 1.0, -1.0, 1.0)
                )
            else:
                velocities = primes[1:4, ..., self.nhx, self.nhy, self.nhz] * mask_ghost
                velocities = velocities - normal * 2 * jnp.einsum(
                    "ijkl, ijkl -> jkl", normal, velocities
                )
                primes = primes.at[1:4, ..., self.nhx, self.nhy, self.nhz].mul(
                    mask_real[..., self.nhx_, self.nhy_, self.nhz_]
                )
        ...
        if self.levelset_type == "FLUID-SOLID-DYNAMIC":
            ...
            primes = primes.at[1:4, self.nhx, self.nhy, self.nhz].add(
                mask_ghost * interface_velocity
            )
        ...
        cons_in_extend = (
            get_conservatives_from_primitives(
                primes[..., self.nhx, self.nhy, self.nhz], self.material_manager
            )
            * mask_extend
        )
    ...
    def transform_to_conservatives(
        self, cons: jnp.DeviceArray, volume_fraction: jnp.DeviceArray
    ) -> jnp.DeviceArray:
        ...
        cons = cons.at[..., self.nhx, self.nhy, self.nhz].mul(
            volume_fraction[..., self.nhx_, self.nhy_, self.nhz_]
        )
        return cons

    def transform_to_volume_averages(
        self, cons: jnp.DeviceArray, volume_fraction: jnp.DeviceArray
    ) -> jnp.DeviceArray:
        ...
        cons = cons.at[..., self.nhx, self.nhy, self.nhz].mul(
            1.0 / (volume_fraction[..., self.nhx_, self.nhy_, self.nhz_] + mask * self.eps)
        )
        ...
        mask = jnp.maximum(
            mask,
            jnp.where(
                (cons[0, ..., self.nhx, self.nhy, self.nhz] <= 0.0)
                | (cons[4, ..., self.nhx, self.nhy, self.nhz] <= 0.0),
                1,
                0,
            ),
        )
    ...
    def weight_cell_face_flux_xi(
        self, flux_xi: jnp.DeviceArray, apertures: jnp.DeviceArray
    ) -> jnp.DeviceArray:
    ...
    def weight_volume_force(
        self, volume_force: jnp.DeviceArray, volume_fraction: jnp.DeviceArray
    ) -> jnp.DeviceArray:
    ...
    def compute_solid_interface_velocity(self, current_time: float) -> jnp.DeviceArray:
        ...
        :rtype: jnp.DeviceArray
        """
        interface_velocity = self.interface_quantity_computer.compute_solid_interface_velocity(
            current_time
        )
    ...
    def compute_interface_quantities(
        self, primes: jnp.DeviceArray, levelset: jnp.DeviceArray, volume_fraction: jnp.DeviceArray
    ) -> Tuple[jnp.DeviceArray, jnp.DeviceArray, float]:
        ...
        normal = self.geometry_calculator.compute_normal(levelset)
        curvature = (
            self.geometry_calculator.compute_curvature(levelset)
        ...
        (
            interface_velocity,
            interface_pressure,
        ) = self.interface_quantity_computer.solve_interface_interaction(primes, normal, curvature)
        interface_quantities = jnp.vstack(
            [jnp.expand_dims(interface_velocity, axis=0), interface_pressure]
        )
        ...
        normal_extend = normal[..., self.nhx_, self.nhy_, self.nhz_] * jnp.sign(
            levelset[self.nhx, self.nhy, self.nhz]
        )
        interface_quantities, max_residual = self.extender_interface.extend(
            interface_quantities,
            normal_extend,
            mask_extend,
            self.CFL_extension_interface,
            self.steps_extension_interface,
        )

        # CUT OFF NARROW BAND
        interface_quantities = (
            interface_quantities[..., self.nhx_, self.nhy_, self.nhz_] * mask_narrow_band
        )
    ...
    def compute_interface_flux_xi(
        ...
        primes: jnp.DeviceArray,
        levelset: jnp.DeviceArray,
        interface_velocity: jnp.DeviceArray,
        interface_pressure: Union[jnp.DeviceArray, None],
        volume_fraction: jnp.DeviceArray,
        apertures: jnp.DeviceArray,
        axis: int,
    ) -> jnp.DeviceArray:
        ...
        interface_flux_xi = self.interface_flux_computer.compute_interface_flux_xi(
            primes, interface_velocity, interface_pressure, volume_fraction, apertures, normal, axis
        )
        return interface_flux_xi

    def compute_levelset_advection_rhs(
        self, levelset: jnp.DeviceArray, interface_velocity: jnp.DeviceArray, axis: int
    ) -> jnp.DeviceArray:
        ...
        # UPWINDING
        velocity = (
            interface_velocity * normal[axis, self.nhx_, self.nhy_, self.nhz_]
            if self.levelset_type == "FLUID-FLUID"
            else interface_velocity[axis]
        )
        ...
        # SUM RHS
        rhs_contribution = (
            -velocity * (mask_L * derivative_L + mask_R * derivative_R) / self.cell_sizes[axis]
        )
    ...
    def reinitialize(
        self, levelset: jnp.DeviceArray, initializer: bool
    ) -> Tuple[jnp.DeviceArray, float]:
        ...
        # REINITIALIZE
        if initializer:
            mask_reinitialize = jnp.ones_like(
                levelset[self.nhx, self.nhy, self.nhz], dtype=jnp.uint32
            )
            mask_residual = jnp.ones_like(levelset[self.nhx, self.nhy, self.nhz], dtype=jnp.uint32)
            levelset, max_residual = self.levelset_reinitializer_init.reinitialize(
                levelset,
                mask_reinitialize,
                mask_residual,
                self.CFL_reinitialization_init,
                self.steps_reinitialization_init,
            )
            ...
            mask_reinitialize = (
                jnp.ones_like(levelset[self.nhx, self.nhy, self.nhz], dtype=jnp.uint32)
                if self.cut_cell_reinitialization
                else jnp.where(
                    jnp.abs(levelset[self.nhx, self.nhy, self.nhz]) / self.smallest_cell_size
                    > jnp.sqrt(2.0) / 2,
                    1,
            ...
            levelset, max_residual = self.levelset_reinitializer.reinitialize(
                levelset,
                mask_reinitialize,
                mask_residual,
                self.CFL_reinitialization,
                self.steps_reinitialization,
            )

        # CUT OFF MASKS
        mask_cut_off_positive = jnp.where(
            levelset[self.nhx, self.nhy, self.nhz] / self.smallest_cell_size
            > self.narrow_band_cutoff,
            1,
            0,
        )
        mask_cut_off_negative = jnp.where(
            levelset[self.nhx, self.nhy, self.nhz] / self.smallest_cell_size
            < -self.narrow_band_cutoff,
            1,
            0,
        ...
        levelset = levelset.at[self.nhx, self.nhy, self.nhz].add(
            mask_cut_off_positive * self.narrow_band_cutoff * self.smallest_cell_size
        )
        levelset = levelset.at[self.nhx, self.nhy, self.nhz].add(
            -mask_cut_off_negative * self.narrow_band_cutoff * self.smallest_cell_size
        )
    ...
    def mixing(
        self,
        cons: jnp.DeviceArray,
        levelset_new: jnp.DeviceArray,
        volume_fraction_new: jnp.DeviceArray,
        volume_fraction_old: jnp.DeviceArray,
    ) -> Tuple[jnp.DeviceArray, jnp.DeviceArray]:
        ...
        if self.levelset_type == "FLUID-FLUID":
            ...
            volume_fraction_new = jnp.stack(
                [volume_fraction_new, 1.0 - volume_fraction_new], axis=0
            )
            volume_fraction_old = jnp.stack(
                [volume_fraction_old, 1.0 - volume_fraction_old], axis=0
            )
        normal_sign = jnp.sign(normal)

        # CREATE SOURCE MASK - SMALL CELLS, NEWLY CREATED, AND VANISHED CELLS
        small_cells = jnp.where(
            (volume_fraction_new < self.volume_fraction_threshold) & (volume_fraction_new > 0.0),
            1,
            0,
        )
        newly_created_cells = jnp.where(
            (volume_fraction_new > 0.0) & (volume_fraction_old == 0.0), 1, 0
        )
        ...
        for i in self.active_axis_indices:
            ...
            volume_fraction_target_at_source = move_target_to_source_ii(
                volume_fraction_new, normal_sign, i
            )
        ...
        if self.dim > 1 and self.mixing_targets > 1:
            ...
            for i, j in self.index_pairs_mixing:
                ...
                volume_fraction_target_at_source = move_target_to_source_ij(
                    volume_fraction_new, normal_sign, i, j
                )
        ...
        if self.dim == 3 and self.mixing_targets == 3:
            ...
            volume_fraction_target_at_source = move_target_to_source_ijk(
                volume_fraction_new, normal_sign
            )
        ...
        mixing_weight_ijk = (
            mixing_weight_ijk / (mixing_weight_sum + self.eps)
            if self.dim == 3 and self.mixing_targets == 3
            else None
        )
        ...
        for i, axis in enumerate(self.active_axis_indices):
            ...
            volume_fraction_target_at_source = move_target_to_source_ii(
                volume_fraction_new, normal_sign, axis
            )
            cons_target_at_source = move_target_to_source_ii(
                cons[..., self.nhx__, self.nhy__, self.nhz__], normal_sign, axis
            )
            # MIXING FLUXES
            M_xi_source = (
                mixing_weight_ii[i]
                / (
                    volume_fraction_new * mixing_weight_ii[i]
                    + volume_fraction_target_at_source
                    + self.eps
                )
                * (
                    cons_target_at_source * volume_fraction_new
                    - cons[..., self.nhx__, self.nhy__, self.nhz__]
                    * volume_fraction_target_at_source
                )
                * mask_source
            )
        ...
        if self.dim > 1 and self.mixing_targets > 1:
            for k, (axis_i, axis_j) in enumerate(self.index_pairs_mixing):
                # MOVE TARGET VALUES TO SOURCE POSITION
                volume_fraction_target_at_source = move_target_to_source_ij(
                    volume_fraction_new, normal_sign, axis_i, axis_j
                )
                cons_target_at_source = move_target_to_source_ij(
                    cons[..., self.nhx__, self.nhy__, self.nhz__], normal_sign, axis_i, axis_j
                )
                # MIXING FLUXES
                M_xi_source = (
                    mixing_weight_ij[k]
                    / (
                        volume_fraction_new * mixing_weight_ij[k]
                        + volume_fraction_target_at_source
                        + self.eps
                    )
                    * (
        ...
        if self.dim == 3 and self.mixing_targets == 3:
            ...
            cons_target_at_source = move_source_to_target_ijk(
                cons[..., self.nhx__, self.nhy__, self.nhz__], normal_sign
            )
            # MIXING FLUXES
            M_xi_source = (
                mixing_weight_ijk
                / (
                    volume_fraction_new * mixing_weight_ijk
                    + volume_fraction_target_at_source
                    + self.eps
                )
                * (
        ...
        cons = cons.at[..., self.nhx, self.nhy, self.nhz].add(
            sum(Mixing_fluxes_source)[..., self.nhx_, self.nhy_, self.nhz_]
            + sum(Mixing_fluxes_target)[..., self.nhx_, self.nhy_, self.nhz_]
        )

        mask_small_cells = jnp.where(
            (cons[0, ..., self.nhx, self.nhy, self.nhz] < 0.0)
            | (cons[4, ..., self.nhx, self.nhy, self.nhz] < 0.0),
            1,
            0,
        ) * jnp.where((volume_fraction_new[..., self.nhx_, self.nhy_, self.nhz_] != 0.0), 1, 0)
    ...
    def compute_primitives_from_conservatives_in_real_fluid(
        ...
        levelset: jnp.DeviceArray,
        volume_fraction: jnp.DeviceArray,
        mask_small_cells: jnp.DeviceArray,
    ) -> jnp.DeviceArray:
        ...
        primes_in_real = (
            get_primitives_from_conservatives(
                cons[..., self.nhx, self.nhy, self.nhz], self.material_manager
            )
            * mask_real
        )
