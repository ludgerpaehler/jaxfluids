...
class GeometryCalculator:
    ...
    def __init__(
        ...
        first_derivative_stencil: SpatialDerivative,
        second_derivative_stencil: SpatialDerivative,
        ...
        self.corner_values_subcell_shape = tuple(
            [
                (domain_information.number_of_cells[i] + 2 * domain_information.nh_geometry) * 2 + 1
        ...
        cell_sizes_IR = (
            jnp.array(self.cell_sizes) * 0.5
            if self.subcell_reconstruction
            else jnp.array(self.cell_sizes)
        )
        if self.dim == 3:
            self.cell_face_area = [
                cell_sizes_IR[1] * cell_sizes_IR[2],
                cell_sizes_IR[0] * cell_sizes_IR[2],
                cell_sizes_IR[0] * cell_sizes_IR[1],
            ]
        elif self.dim == 2:
        ...
        if "".join(domain_information.active_axis) == "x":
            ...
            self.levelset_center_value_slices = [np.s_[:-1, :, :], np.s_[1:, :, :]]
        ...
        # BIT TO INT
        self.bitset_to_int = lambda bitset: jnp.sum(
            jnp.array([bitset[i] * 2**i for i in range(len(bitset))]), axis=0
        )
        ...
        self.PM = lambda corner_values: corner_values[0] / (
            corner_values[0] + corner_values[1] + self.eps
        )
        self.MP = lambda corner_values: corner_values[1] / (
            corner_values[1] + corner_values[0] + self.eps
        )
        ...
        self.PMMM = lambda corner_values: 0.5 * (
            corner_values[0]
            / (corner_values[0] + corner_values[3] + self.eps)
            * corner_values[0]
            / (corner_values[0] + corner_values[1] + self.eps)
        )
        self.MPMM = lambda corner_values: 0.5 * (
            corner_values[1]
            / (corner_values[1] + corner_values[0] + self.eps)
            * corner_values[1]
            / (corner_values[1] + corner_values[2] + self.eps)
        )
        self.MMPM = lambda corner_values: 0.5 * (
            corner_values[2]
            / (corner_values[2] + corner_values[1] + self.eps)
            * corner_values[2]
            / (corner_values[2] + corner_values[3] + self.eps)
        )
        self.MMMP = lambda corner_values: 0.5 * (
            corner_values[3]
            / (corner_values[3] + corner_values[1] + self.eps)
            * corner_values[3]
            / (corner_values[3] + corner_values[2] + self.eps)
        )
        self.PPMM = lambda corner_values: 0.5 * (
            corner_values[0] / (corner_values[0] + corner_values[3] + self.eps)
            + corner_values[1] / (corner_values[1] + corner_values[2] + self.eps)
        )
        self.PMMP = lambda corner_values: 0.5 * (
            corner_values[0] / (corner_values[0] + corner_values[1] + self.eps)
            + corner_values[3] / (corner_values[3] + corner_values[2] + self.eps)
        )
        ...
        self.PMPM = lambda corner_values: (jnp.sum(corner_values, axis=0) > 0.0) * (
            1.0 - (self.MPMM(corner_values) + self.MMMP(corner_values))
        ) + (np.sum(corner_values, axis=0) <= 0.0) * (
            self.PMMM(corner_values) + self.MMPM(corner_values)
        )
        self.MPMP = lambda corner_values: (jnp.sum(corner_values, axis=0) > 0.0) * (
            1.0 - (self.PMMM(corner_values) + self.MMPM(corner_values))
        ) + (np.sum(corner_values, axis=0) <= 0.0) * (
            self.MPMM(corner_values) + self.MMMP(corner_values)
        )

        self.index_pairs = [(0, 1), (0, 2), (1, 2)]

    def linear_interface_reconstruction(
        ...
        for i in range(3):
            apertures.append(
                self.compute_apertures(corner_values, i) if i in self.active_axis_indices else None
            )
        volume_fraction = self.compute_volume_fraction(corner_values, apertures)
        if self.subcell_reconstruction:
            volume_fraction = self.interpolation_factor[self.dim - 1] * sum(
                [
                    volume_fraction[slices]
                    for slices in self.volume_fraction_subcell_interpolation_slices
                ]
            )
            for i in self.active_axis_indices:
                apertures[i] = (
                    2
                    * self.interpolation_factor[self.dim - 1]
                    * sum(
                        [
                            apertures[i][slices]
                            for slices in self.aperture_subcell_interpolation_slices[i]
                        ]
                    )
                )
    ...
    def compute_corner_values(self, levelset: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        corner_values = factor * sum(
            [levelset[slices] for slices in self.single_cell_interpolation_slices]
        )
        if self.subcell_reconstruction:
            corner_values_subcell = jnp.zeros(self.corner_values_subcell_shape)
            interpolations = []
            for interpolation_slices, set_slices in zip(
                self.subcell_interpolation_slices, self.set_subcell_buffer_slices
            ):
                factor = 1.0 / len(interpolation_slices)
            ...
            corner_values_subcell = corner_values_subcell.at[
                self.set_subcell_buffer_slices[-2]
            ].set(corner_values)
            corner_values_subcell = corner_values_subcell.at[
                self.set_subcell_buffer_slices[-1]
            ].set(levelset[self.nhx__, self.nhy__, self.nhz__])
    ...
    def compute_apertures(self, corner_values: jnp.DeviceArray, axis: int) -> jnp.DeviceArray:
        ...
        if self.dim == 1:
            ...
            corner_values_for_apertures = jnp.stack(
                [
                    corner_values[self.rearrange_for_aperture_lambdas_slices[axis][0]],
            ...
            apertures = (
                self.MM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 0, 1.0, 0.0)
                + self.PM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 1, 1.0, 0.0)
                + self.MP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 2, 1.0, 0.0)
                + self.PP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 3, 1.0, 0.0)
            )
            ...
            corner_values_for_apertures = jnp.stack(
                [
                    ...
                    corner_values[self.rearrange_for_aperture_lambdas_slices[axis][3]],
                ],
                axis=0,
            )
            corner_values_for_apertures_sign = jnp.where(corner_values_for_apertures > 0.0, 1, 0)
            apertures = (
                self.MMMM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 0, 1.0, 0.0)
                + self.PMMM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 1, 1.0, 0.0)
                + self.MPMM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 2, 1.0, 0.0)
                + self.PPMM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 3, 1.0, 0.0)
                + self.MMPM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 4, 1.0, 0.0)
                + self.PMPM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 5, 1.0, 0.0)
                + self.MPPM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 6, 1.0, 0.0)
                + self.PPPM(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 7, 1.0, 0.0)
                + self.MMMP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 8, 1.0, 0.0)
                + self.PMMP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 9, 1.0, 0.0)
                + self.MPMP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 10, 1.0, 0.0)
                + self.PPMP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 11, 1.0, 0.0)
                + self.MMPP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 12, 1.0, 0.0)
                + self.PMPP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 13, 1.0, 0.0)
                + self.MPPP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 14, 1.0, 0.0)
                + self.PPPP(jnp.abs(corner_values_for_apertures))
                * jnp.where(self.bitset_to_int(corner_values_for_apertures_sign) == 15, 1.0, 0.0)
            )
    ...
    def compute_volume_fraction(
        self, corner_values: jnp.DeviceArray, apertures: List
    ) -> jnp.DeviceArray:
        ...
        :rtype: jnp.DeviceArray
        """
        levelset_center_value = self.interpolation_factor[self.dim - 1] * sum(
            [corner_values[slices] for slices in self.levelset_center_value_slices]
        )
        interface_length = jnp.sqrt(
            sum(
                [
                    (
                        (
                            apertures[i][self.axis_slices_apertures[i][0]]
                            - apertures[i][self.axis_slices_apertures[i][1]]
                        )
                        * self.cell_face_area[i]
                    )
                    ** 2
                    for i in self.active_axis_indices
                ]
    ...
    def compute_normal(self, levelset: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        for i in range(3):
            normal.append(
                self.derivative_stencil.derivative_xi(levelset, self.cell_sizes[i], i)
                if i in self.active_axis_indices
                else jnp.zeros(levelset[self.nhx__, self.nhy__, self.nhz__].shape)
            )
    ...
    def compute_curvature(self, levelset: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        for i in range(3):
            second_derivative_ii.append(
                self.second_derivative_stencil.derivative_xi(levelset, self.cell_sizes[i], i)
                if i in self.active_axis_indices
                else jnp.zeros(levelset[self.nhx__, self.nhy__, self.nhz__].shape)
            )
        second_derivative_ij = []
        for i, j in self.index_pairs:
            second_derivative_ij.append(
                self.second_derivative_stencil.derivative_xi_xj(
                    levelset, self.cell_sizes[i], self.cell_sizes[j], i, j
                )
                if i in self.active_axis_indices and j in self.active_axis_indices
                else jnp.zeros(levelset[self.nhx__, self.nhy__, self.nhz__].shape)
            )
        ...
        curvature = (
            second_derivative_ii[0] + second_derivative_ii[1] + second_derivative_ii[2]
        ) / gradient_length - (
            gradient[0] ** 2 * second_derivative_ii[0]
            + gradient[1] ** 2 * second_derivative_ii[1]
            + gradient[2] ** 2 * second_derivative_ii[2]
            + 2
            * (
                gradient[0] * gradient[1] * second_derivative_ij[0]
                + gradient[0] * gradient[2] * second_derivative_ij[1]
                + gradient[1] * gradient[2] * second_derivative_ij[2]
            )
        ) / gradient_length**3
        ...
        curvature = (
            self.dim
            * curvature
            / (self.dim - levelset[self.nhx__, self.nhy__, self.nhz__] * curvature)
        )
