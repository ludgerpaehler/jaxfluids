...
class LevelsetReinitializer:
    ...
    def reinitialize(
        ...
        mask_reinitialize: jnp.DeviceArray,
        mask_residual: jnp.DeviceArray,
        CFL: float,
        steps: int,
        ...
        for i in range(steps):
            levelset, residual = self.do_integration_step(
                levelset, levelset_0, mask_reinitialize, timestep_size
            )
    ...
    def compute_rhs(
        self, levelset: jnp.DeviceArray, levelset_0: jnp.DeviceArray, mask: jnp.DeviceArray
    ) -> Tuple[jnp.DeviceArray, jnp.DeviceArray]:
        ...
        if self.dim == 1:
            if self.active_axis_indices == [0]:
                distance = (
                    ...
                    * levelset_0[self.nhx, self.nhy, self.nhz]
                    / jnp.abs(
                        levelset_0[self.nh + 1 : -self.nh + 1, self.nhy, self.nhz]
                        - levelset_0[self.nh - 1 : -self.nh - 1, self.nhy, self.nhz]
                        + self.eps
                    )
                )
                mask_cut_cells = jnp.where(
                    (
                        levelset_0[self.nhx, self.nhy, self.nhz]
                        * levelset_0[self.nh - 1 : -self.nh - 1, self.nhy, self.nhz]
                        < 0
                ...
                distance = (
                    ...
                    * levelset_0[self.nhx, self.nhy, self.nhz]
                    / jnp.abs(
                        levelset_0[self.nhx, self.nh + 1 : -self.nh + 1, self.nhz]
                        - levelset_0[self.nhx, self.nh - 1 : -self.nh - 1, self.nhz]
                        + self.eps
                    )
                )
                ...
                distance = (
                    ...
                    * levelset_0[self.nhx, self.nhy, self.nhz]
                    / jnp.abs(
                        levelset_0[self.nhx, self.nhy, self.nh + 1 : -self.nh + 1]
                        - levelset_0[self.nhx, self.nhy, self.nh - 1 : -self.nh - 1]
                        + self.eps
                    )
                )
        ...
        for axis in self.active_axis_indices:
            derivatives_L.append(
                self.derivative_stencil.derivative_xi(
                    levelset, self.cell_sizes[axis], axis, 0, levelset_0, distance
                )
            )
            derivatives_R.append(
                self.derivative_stencil.derivative_xi(
                    levelset, self.cell_sizes[axis], axis, 1, levelset_0, distance
                )
            )
        ...
        smooth_sign = (
            mask_cut_cells * distance / self.smallest_cell_size + (1 - mask_cut_cells) * sign
        )

        # GODUNOV HAMILTONIAN
        godunov_hamiltonian = 0.0
        for der_L, der_R in zip(derivatives_L, derivatives_R):
            godunov_hamiltonian += jnp.maximum(
                jnp.maximum(0.0, sign * der_L) ** 2, jnp.minimum(0.0, sign * der_R) ** 2
            )
