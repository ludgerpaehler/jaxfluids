...
class InterfaceFluxComputer:
    ...
    def __init__(
        ...
        self.derivative_stencil: SpatialDerivative = (
            DICT_FIRST_DERIVATIVE_CENTER[
                numerical_setup["conservatives"]["dissipative_fluxes"]["derivative_stencil_center"]
    ...
    def compute_interface_flux_xi(
        ...
        interface_pressure: jnp.DeviceArray,
        ...
        if self.levelset_type in ["FLUID-SOLID-STATIC", "FLUID-SOLID-DYNAMIC"]:
            ...
            if self.is_convective_flux:
                ...
                convective_flux_xi_momentum = primes[4, self.nhx, self.nhy, self.nhz] * (
                    apertures[self.aperture_slices[axis][0]][..., self.nhx_, self.nhy_, self.nhz_]
                    - apertures[self.aperture_slices[axis][1]][..., self.nhx_, self.nhy_, self.nhz_]
                )
                interface_flux_xi = interface_flux_xi.at[axis + 1].add(convective_flux_xi_momentum)

                # ENERGY CONTRIBUTION
                if self.levelset_type == "FLUID-SOLID-DYNAMIC":
                    convective_flux_xi_energy = (
                        convective_flux_xi_momentum * interface_velocity[axis]
                    )
            ...
            if self.is_viscous_flux:
                ...
                velocity_gradient = jnp.stack(
                    [
                        self.derivative_stencil.derivative_xi(velocity, self.cell_sizes[k], k)
                ...
                mu_1 = self.material_manager.get_dynamic_viscosity(
                    self.material_manager.get_temperature(
                        primes[4, self.nhx, self.nhy, self.nhz],
                        primes[0, self.nhx, self.nhy, self.nhz],
                    )
                )
                ...
                # COMPUTE TAU
                tau_i = [
                    mu_1 * (velocity_gradient[axis, 0] + velocity_gradient[0, axis])
                    if axis in self.active_axis_indices and 0 in self.active_axis_indices
                    else jnp.zeros(velocity_gradient.shape[2:]),
                    mu_1 * (velocity_gradient[axis, 1] + velocity_gradient[1, axis])
                    if axis in self.active_axis_indices and 1 in self.active_axis_indices
                    else jnp.zeros(velocity_gradient.shape[2:]),
                    mu_1 * (velocity_gradient[axis, 2] + velocity_gradient[2, axis])
                    if axis in self.active_axis_indices and 2 in self.active_axis_indices
                    else jnp.zeros(velocity_gradient.shape[2:]),
                ]
                tau_i[axis] += mu_2 * sum(
                    [velocity_gradient[k, k] for k in self.active_axis_indices]
                )
                tau_i = -jnp.stack(tau_i) * (
                    apertures[self.aperture_slices[axis][0]][..., self.nhx_, self.nhy_, self.nhz_]
                    - apertures[self.aperture_slices[axis][1]][..., self.nhx_, self.nhy_, self.nhz_]
                )
            ...
            # CONVECTIVE FLUX
            if self.is_convective_flux:
                convective_flux_xi_momentum = interface_pressure * (
                    apertures[self.aperture_slices[axis][0]][..., self.nhx_, self.nhy_, self.nhz_]
                    - apertures[self.aperture_slices[axis][1]][..., self.nhx_, self.nhy_, self.nhz_]
                )
                convective_flux_xi_energy = (
                    convective_flux_xi_momentum
                    * interface_velocity
                    * normal[axis][..., self.nhx_, self.nhy_, self.nhz_]
                )
                interface_flux_xi = interface_flux_xi.at[axis + 1].add(convective_flux_xi_momentum)
                interface_flux_xi = interface_flux_xi.at[4].add(convective_flux_xi_energy)
            ...
            if self.is_viscous_flux:
                ...
                velocity_0, velocity_1 = (
                    primes[1:4, 0, self.nhx__, self.nhy__, self.nhz__],
                    primes[1:4, 1, self.nhx__, self.nhy__, self.nhz__],
                )
                real_velocity = velocity_0 * volume_fraction[0] + velocity_1 * volume_fraction[1]
                velocity_gradient = jnp.stack(
                    [
                        self.derivative_stencil.derivative_xi(real_velocity, self.cell_sizes[k], k)
                        if k in self.active_axis_indices
                        else jnp.zeros(real_velocity[:, self.nhx_, self.nhy_, self.nhz_].shape)
                        for k in range(3)
                    ],
                    axis=1,
                )
                ...
                mu_1_interface = (
                    ...
                    * mu_1[1]
                    / (
                        volume_fraction[0, self.nhx_, self.nhy_, self.nhz_] * mu_1[1]
                        + volume_fraction[1, self.nhx_, self.nhy_, self.nhz_] * mu_1[0]
                    )
                )
                mu_2_interface = (
                    mu_2[0]
                    * mu_2[1]
                    / (
                        volume_fraction[0, self.nhx_, self.nhy_, self.nhz_] * mu_2[1]
                        + volume_fraction[1, self.nhx_, self.nhy_, self.nhz_] * mu_2[0]
                    )
                )

                # COMPUTE TAU
                tau_i = [
                    mu_1_interface * (velocity_gradient[axis, 0] + velocity_gradient[0, axis])
                    if axis in self.active_axis_indices and 0 in self.active_axis_indices
                    else jnp.zeros(velocity_gradient.shape[2:]),
                    mu_1_interface * (velocity_gradient[axis, 1] + velocity_gradient[1, axis])
                    if axis in self.active_axis_indices and 1 in self.active_axis_indices
                    else jnp.zeros(velocity_gradient.shape[2:]),
                    mu_1_interface * (velocity_gradient[axis, 2] + velocity_gradient[2, axis])
                    if axis in self.active_axis_indices and 2 in self.active_axis_indices
                    else jnp.zeros(velocity_gradient.shape[2:]),
                ]
                tau_i[axis] += mu_2_interface * sum(
                    [velocity_gradient[k, k] for k in self.active_axis_indices]
                )
                tau_i = -jnp.expand_dims(jnp.stack(tau_i), axis=1) * (
                    apertures[self.aperture_slices[axis][0]][..., self.nhx_, self.nhy_, self.nhz_]
                    - apertures[self.aperture_slices[axis][1]][..., self.nhx_, self.nhy_, self.nhz_]
                )
                ...
                viscid_flux_xi_energy = 0
                for k in self.active_axis_indices:
                    viscid_flux_xi_energy += (
                        tau_i[k] * interface_velocity * normal[k, self.nhx_, self.nhy_, self.nhz_]
                    )
                interface_flux_xi = interface_flux_xi.at[4].add(viscid_flux_xi_energy)
