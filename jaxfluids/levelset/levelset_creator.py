...
class LevelsetCreator:
    ...
    def __init__(
        ...
        initial_levelset: Union[str, List],
        narrow_band_cutoff: int,
    ) -> None:
    ...
    def get_circle(self, radius: float, position: List) -> jnp.DeviceArray:
        ...
        levelset = -radius + jnp.sqrt(
            (mesh_grid[0] - position[0]) ** 2 + (mesh_grid[1] - position[1]) ** 2
        )
        return levelset

    def get_rectangle(
        self, length: float, position: List, height: float = None, radius: float = None
    ) -> jnp.DeviceArray:
        ...
        levelset = (mesh_grid[1] - (position[1] + height / 2.0)) * (
            (mesh_grid[1] > line_1_slope * mesh_grid[0] + line_1_offset)
            & (mesh_grid[1] > line_2_slope * mesh_grid[0] + line_2_offset)
        )
        levelset += -(mesh_grid[1] - (position[1] - height / 2.0)) * (
            (mesh_grid[1] < line_1_slope * mesh_grid[0] + line_1_offset)
            & (mesh_grid[1] < line_2_slope * mesh_grid[0] + line_2_offset)
        )
        levelset += (mesh_grid[0] - (position[0] + length / 2.0)) * (
            (mesh_grid[1] <= line_1_slope * mesh_grid[0] + line_1_offset)
            & (mesh_grid[1] >= line_2_slope * mesh_grid[0] + line_2_offset)
        )
        levelset += -(mesh_grid[0] - (position[0] - length / 2.0)) * (
        ...
        if radius:
            levelset *= jnp.invert(
                (mesh_grid[0] > edge_tr[0] - radius) & (mesh_grid[1] > edge_tr[1] - radius)
            )
            levelset *= jnp.invert(
                (mesh_grid[0] < edge_tl[0] + radius) & (mesh_grid[1] > edge_tl[1] - radius)
            )
            levelset *= jnp.invert(
                (mesh_grid[0] < edge_bl[0] + radius) & (mesh_grid[1] < edge_bl[1] + radius)
            )
            levelset *= jnp.invert(
                (mesh_grid[0] > edge_br[0] - radius) & (mesh_grid[1] < edge_br[1] + radius)
            )

            levelset += self.get_circle(radius, edge_tr + jnp.array([-1.0, -1.0]) * radius) * (
                (mesh_grid[0] > edge_tr[0] - radius) & (mesh_grid[1] > edge_tr[1] - radius)
            )
            levelset += self.get_circle(radius, edge_tl + jnp.array([1.0, -1.0]) * radius) * (
                (mesh_grid[0] < edge_tl[0] + radius) & (mesh_grid[1] > edge_tl[1] - radius)
            )
            levelset += self.get_circle(radius, edge_bl + jnp.array([1.0, 1.0]) * radius) * (
                (mesh_grid[0] < edge_bl[0] + radius) & (mesh_grid[1] < edge_bl[1] + radius)
            )
            levelset += self.get_circle(radius, edge_br + jnp.array([-1.0, 1.0]) * radius) * (
                (mesh_grid[0] > edge_br[0] - radius) & (mesh_grid[1] < edge_br[1] + radius)
            )
    ...
    def create_levelset(self) -> jnp.DeviceArray:
        ...
        levelset_buffer = levelset_cutoff * jnp.ones(
            (
                self.nx + 2 * self.nh if self.nx > 1 else self.nx,
                self.ny + 2 * self.nh if self.ny > 1 else self.ny,
                self.nz + 2 * self.nh if self.nz > 1 else self.nz,
            )
        )
        ...
        # FROM BUILDING BLOCKS
            ...
            for levelset_object in self.initial_levelset:
                levelset = self.shape_function_dict[levelset_object["shape"]](
                    **levelset_object["parameters"]
                )
                levelset = self.unit_handler.non_dimensionalize(levelset, "length")
                ...
                levelset_buffer = levelset_buffer.at[self.nhx, self.nhy, self.nhz].add(
                    levelset * mask
                )
