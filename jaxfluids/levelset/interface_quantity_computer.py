    ...
    def compute_solid_interface_velocity(self, current_time: float) -> jnp.DeviceArray:
        ...
        if type(self.solid_interface_velocity) == list:
            ...
            for velocity_object in self.solid_interface_velocity:
                ...
                velocity_array = jnp.stack(
                    [
                        velocity_tuple[i]
                        if i in self.active_axis_indices
                        else jnp.zeros_like(velocity_tuple[0])
                        for i in range(3)
                    ],
                    axis=0,
                )
        ...
        # LAMBDA FUNCTION
            ...
            solid_interface_velocity = jnp.stack(
                [
                    velocity_tuple[i]
                    if i in self.active_axis_indices
                    else jnp.zeros_like(velocity_tuple[0])
        ...
        solid_interface_velocity = self.unit_handler.non_dimensionalize(
            solid_interface_velocity, "velocity"
        )

        return solid_interface_velocity

    def solve_interface_interaction(
        self, primes: jnp.DeviceArray, normal: jnp.DeviceArray, curvature: jnp.DeviceArray
        ...
        velocity_normal_projection = jnp.einsum(
            "ijklm, ijklm -> jklm", primes[1:4], jnp.expand_dims(normal, axis=1)
        )
        ...
        interface_velocity = (
            impendance[1] * velocity_normal_projection[1]
            + impendance[0] * velocity_normal_projection[0]
            + pressure[1]
            - pressure[0]
            - delta_p
        ) * inverse_impendace_sum
        interface_pressure_positive = (
            impendance[1] * pressure[0]
            + impendance[0] * (pressure[1] - delta_p)
            + impendance[0]
            * impendance[1]
            * (velocity_normal_projection[1] - velocity_normal_projection[0])
        ) * inverse_impendace_sum
        interface_pressure_negative = (
            impendance[1] * (pressure[0] + delta_p)
            + impendance[0] * pressure[1]
            + impendance[0]
            * impendance[1]
            * (velocity_normal_projection[1] - velocity_normal_projection[0])
        ) * inverse_impendace_sum

        interface_pressure = jnp.stack(
            [interface_pressure_positive, interface_pressure_negative], axis=0
        )
