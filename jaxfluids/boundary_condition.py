...
class BoundaryCondition:
    ...
    def __init__(
        ...
        boundary_types: Dict,
        wall_velocity_functions: Dict,
        dirichlet_functions: Dict,
        neumann_functions: Dict,
        levelset_type: str,
    ) -> None:
        ...
        self.cell_sizes = {
            "east": domain_information.cell_sizes[0],
            "west": domain_information.cell_sizes[0],
            "north": domain_information.cell_sizes[1],
            "south": domain_information.cell_sizes[1],
            "top": domain_information.cell_sizes[2],
            "bottom": domain_information.cell_sizes[2],
        }

        self.coordinates_plane = {
            "east": {
                "y": domain_information.cell_centers[1],
                "z": domain_information.cell_centers[2],
            },
            "west": {
                "y": domain_information.cell_centers[1],
                "z": domain_information.cell_centers[2],
            ...
            "north": {
                "x": domain_information.cell_centers[0],
                "z": domain_information.cell_centers[2],
            },
            "south": {
                "x": domain_information.cell_centers[0],
    ...
    def assign_corners(self) -> Dict:
        ...
        boundary_types["levelset"] = (
            self.boundary_types_levelset if self.levelset_type != None else None
        )
        ...
        for key in corners:
            ...
            for corner, combinations in corners[key].items():
                ...
                if np.array(
                    [bound in ["dirichlet", "neumann", "wall"] for bound in [boundary1, boundary2]]
                ).all():
                    boundary1, boundary2 = "any", "any"
                elif (
                    boundary1 in ["dirichlet", "neumann", "wall", "symmetry"]
                    and boundary2 == "periodic"
                ):
                    boundary1 = "any"
                elif (
                    boundary1 in ["dirichlet", "neumann", "wall", "periodic"]
                    and boundary2 == "symmetry"
                ):
                    boundary1 = "any"
                elif (
                    boundary2 in ["dirichlet", "neumann", "wall", "symmetry"]
                    and boundary1 == "periodic"
                ):
                    boundary2 = "any"
                elif (
                    boundary2 in ["dirichlet", "neumann", "wall", "periodic"]
                    and boundary1 == "symmetry"
                ):
                    boundary2 = "any"
    ...
    def fill_boundary_primes(
        self, cons: jnp.DeviceArray, primes: jnp.DeviceArray, current_time: float
        ...
        for boundary_location, boundary_types in self.boundary_types_primes.items():
            ...
            for b_type, b_range in zip(b_types, b_ranges):
                ...
                if b_type == "wall":
                    ...
                    cons, primes = self.wall(
                        cons, primes, boundary_location, function, current_time, b_range
                    )
                    wall_counter += 1
    ...
    def fill_corners_levelset(self, levelset: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        for location_fill, combinations in self.corners["levelset"].items():
            ...
            if combinations == "any_any":
                ...
                halo = 0.5 * (
                    levelset[slice_retrieve1][:: flip1[0], :: flip1[1], :: flip1[2]]
                    + levelset[slice_retrieve2][:: flip2[0], :: flip2[1], :: flip2[2]]
                )
                ...
                levelset = levelset.at[slice_fill].set(
                    levelset[slice_retrieve][..., :: flip[0], :: flip[1], :: flip[2]]
                )
    ...
    def fill_corners_primes(self, primes: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        for location_fill, combinations in self.corners["primes"].items():
            ...
            if combinations == "any_any":
                ...
                halo = 0.5 * (
                    primes[slice_retrieve1][..., :: flip1[0], :: flip1[1], :: flip1[2]]
                    + primes[slice_retrieve2][..., :: flip2[0], :: flip2[1], :: flip2[2]]
                )
                ...
                if "symmetry" in combinations.split("_"):
                    ...
                    primes = primes.at[fill_flip].set(
                        -primes[retrieve_flip][..., :: flip[0], :: flip[1], :: flip[2]]
                    )
                    primes = primes.at[fill_noflip].set(
                        primes[retrieve_noflip][..., :: flip[0], :: flip[1], :: flip[2]]
                    )
                    ...
                    primes = primes.at[slice_fill].set(
                        primes[slice_retrieve][..., :: flip[0], :: flip[1], :: flip[2]]
                    )
    ...
    def wall(
        ...
        for velocity in ["u", "v", "w"]:
            if velocity in functions.keys():
                if type(functions[velocity]) == types.LambdaType:
                    ...
                    wall_velocity[velocity] = functions[velocity]
                wall_velocity[velocity] = self.unit_handler.non_dimensionalize(
                    wall_velocity[velocity], "velocity"
                )
            else:
        ...
        halos_prime = jnp.vstack(
            [
                primes[(jnp.s_[0:1],) + slices_retrieve],
                u_halo,
                v_halo,
                w_halo,
    ...
    def dirichlet(
        ...
        primes: jnp.DeviceArray,
        location: str,
        functions: Union[Callable, float],
        current_time: float,
        b_range: List,
    ) -> Tuple[jnp.DeviceArray, jnp.DeviceArray]:
        ...
        coordinates = [
            self.coordinates_plane[location].get(axis)[
                int(b_range[0] * self.number_of_cells[self.spatial_axis_to_index[axis]]) : int(
                    b_range[1] * self.number_of_cells[self.spatial_axis_to_index[axis]]
                )
            ]
            for axis in self.active_axis
            if self.coordinates_plane[location].get(axis) != None
        ]
        coordinates_name = [
            axis for axis in self.active_axis if self.coordinates_plane[location].get(axis) != None
        ]
        axis_to_expand = [axis for axis in ["x", "y", "z"] if axis not in coordinates_name]

        # DIMENSIONALIZE FOR LAMBDA FUNCTION
        mesh_grid = jnp.meshgrid(
            *[self.unit_handler.dimensionalize(coord, "length") for coord in coordinates],
            indexing="ij",
        )
        ...
        for prime_state in functions:
            ...
            if type(func) in [float, np.float64, np.float32]:
                ...
                assert (
                    False
                ), "Dirichlet boundary values must be lambda function or python/numpy float"
    ...
    def neumann(
        ...
        for i, prime_state in enumerate(functions):
            ...
            if type(func) in [float, np.float64, np.float32]:
                ...
                assert (
                    False
                ), "Neumann boundary values must be lambda function or python/numpy float"
            ...
            halos = (
                primes[(i,) + slices_retrieve]
                + self.upwind_difference_sign[location] * neumann_value * self.cell_sizes[location]
            )
            halos_prime_list.append(halos)
    ...
    def symmetry(
        self, cons: jnp.DeviceArray, primes: jnp.DeviceArray, location: str, b_range: List
    ) -> Tuple[jnp.DeviceArray, jnp.DeviceArray]:
        ...
        cons = cons.at[(self.symmetry_indices[axis][0],) + slices_fill].set(
            cons[(self.symmetry_indices[axis][0],) + slices_retrieve]
        )
        cons = cons.at[(self.symmetry_indices[axis][1],) + slices_fill].set(
            -cons[(self.symmetry_indices[axis][1],) + slices_retrieve]
        )
        primes = primes.at[(self.symmetry_indices[axis][0],) + slices_fill].set(
            primes[(self.symmetry_indices[axis][0],) + slices_retrieve]
        )
        primes = primes.at[(self.symmetry_indices[axis][1],) + slices_fill].set(
            -primes[(self.symmetry_indices[axis][1],) + slices_retrieve]
        )
