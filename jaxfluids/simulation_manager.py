...
from jaxfluids.materials.material_manager import Material, MaterialManager
from jaxfluids.solvers.riemann_solvers.eigendecomposition import \
    Eigendecomposition
...
class SimulationManager:
    ...
    def __init__(self, input_reader: InputReader) -> None:
        ...
        self.eps = (
            ...
            if self.numerical_setup["output"]["is_double_precision_compute"]
        ...
        self.time_integrator: TimeIntegrator = DICT_TIME_INTEGRATION[
            self.numerical_setup["conservatives"]["time_integration"]["integrator"]
        ](
            nh=self.domain_information.nh_conservatives,
            inactive_axis=self.domain_information.inactive_axis,
        )
        ...
        if self.input_reader.active_forcings:
            self.forcings_computer = Forcing(
                ...
                levelset_type=self.input_reader.levelset_type,
                is_mass_flow_forcing=self.numerical_setup["active_forcings"][
                    "is_mass_flow_forcing"
                ],
                is_temperature_forcing=self.numerical_setup["active_forcings"][
        ...
        self.output_writer = OutputWriter(
            ...
            derivative_stencil_conservatives=DICT_FIRST_DERIVATIVE_CENTER[
                self.numerical_setup["output"]["derivative_stencil"]
            ](
                nh=self.domain_information.nh_conservatives,
                inactive_axis=self.domain_information.inactive_axis,
            ),
            derivative_stencil_geometry=DICT_FIRST_DERIVATIVE_CENTER[
                self.numerical_setup["output"]["derivative_stencil"]
            ](
                nh=self.domain_information.nh_geometry,
                inactive_axis=self.domain_information.inactive_axis,
            )
            if self.input_reader.levelset_type != None
            else None,
    ...
    def simulate(
        self, buffer_dictionary: Dict[str, Dict[str, Union[jnp.DeviceArray, float]]]
    ) -> None:
    ...
    def advance(
        ...
        while current_time < self.end_time - self.eps_time:
            ...
            if self.input_reader.active_forcings:
                if self.numerical_setup["active_forcings"]["is_turb_hit_forcing"]:
                    material_fields, _, _ = self.do_integration_step(
                        **buffer_dictionary["material_fields"], **buffer_dictionary["time_control"]
                    )
            ...
            if self.input_reader.levelset_type != None:
                reinitialize = (
                    True
                    if current_step % self.levelset_handler.interval_reinitialization == 0
                    else False
                )
            else:
                reinitialize = False
            ...
            mean_wall_clock_step = (
                (wall_clock_step + mean_wall_clock_step * (current_step - 1)) / current_step
                if current_step > 3
                else wall_clock_step
            )
            mean_wall_clock_step_cell = (
                (wall_clock_step_cell + mean_wall_clock_step_cell * (current_step - 1))
                / current_step
                if current_step > 3
                else wall_clock_step_cell
            )
            ...
            if self.input_reader.levelset_type != None:
                print_list += [
                    "RESIDUAL EXTENSION PRIMES      = %4.4e" % residuals["extension_primes"]
                ]
            if self.input_reader.levelset_type == "FLUID-FLUID":
                print_list += [
                    "RESIDUAL EXTENSION INTERFACE   = %4.4e" % residuals["extension_interface"]
                ]
                print_list += [
                    "RESIDUAL REINITIALIZATION      = %4.4e" % residuals["reinitialization"]
                ]
    ...
    def do_integration_step(
        ...
        timestep_size: float,
        current_time: float,
        levelset: Union[jnp.DeviceArray, None] = None,
        volume_fraction: Union[jnp.DeviceArray, None] = None,
        apertures: Union[List, None] = None,
        reinitialize: bool = False,
        forcings_dictionary: Union[Dict, None] = None,
        ...
        **kwargs,
        ...
        if self.time_integrator.no_stages > 1:
            init_cons = (
                self.levelset_handler.transform_to_conservatives(cons, volume_fraction)
                if self.input_reader.levelset_type != None
                else jnp.array(cons, copy=True)
            )
            init_levelset = (
                jnp.array(levelset, copy=True)
        ...
        for stage in range(self.time_integrator.no_stages):
            ...
            if stage > 0:
                ...
                if self.input_reader.levelset_type in ["FLUID-FLUID", "FLUID-SOLID-DYNAMIC"]:
                    levelset = self.time_integrator.prepare_buffer_for_integration(
                        levelset, init_levelset, stage
                    )

            # INTEGRATE
            cons = self.time_integrator.integrate(cons, rhs_cons, timestep_size, stage)
            if self.input_reader.levelset_type in ["FLUID-FLUID", "FLUID-SOLID-DYNAMIC"]:
                levelset_new = self.time_integrator.integrate(
                    levelset, rhs_levelset, timestep_size, stage
                )

                # REINITIALIZE
                if (
                    self.input_reader.levelset_type == "FLUID-FLUID"
                    and stage == self.time_integrator.no_stages - 1
                    and reinitialize
                ):
                    levelset_new, residual_reinit = self.levelset_handler.reinitialize(
                        levelset_new, False
                    )
                else:
                    residual_reinit = 0.0
                ...
                (
                    volume_fraction_new,
                    apertures_new,
                ) = self.levelset_handler.compute_volume_fraction_and_apertures(levelset_new)

            elif self.input_reader.levelset_type == "FLUID-SOLID-STATIC":
                levelset_new, volume_fraction_new, apertures_new = (
                    levelset,
                    volume_fraction,
                    apertures,
                )
            ...
            current_time_stage = (
                current_time + timestep_size * self.time_integrator.timestep_increment_factor[stage]
            )

            # MIXING AND PRIME EXTENSION
            if self.input_reader.levelset_type != None:
                cons, mask_small_cells = self.levelset_handler.mixing(
                    cons, levelset_new, volume_fraction_new, volume_fraction
                )
                cons = self.levelset_handler.transform_to_volume_averages(cons, volume_fraction_new)
                primes = self.levelset_handler.compute_primitives_from_conservatives_in_real_fluid(
                    cons, primes, levelset_new, volume_fraction_new, mask_small_cells
                )
                cons, primes, residual_primes = self.levelset_handler.extend_primes(
                    cons,
                    primes,
                    levelset_new,
                    volume_fraction_new,
        ...
        residuals = {
            "extension_primes": residual_primes,
            "extension_interface": residual_interface,
            "reinitialization": residual_reinit,
        }
    ...
    def compute_timestep(
        ...
        **kwargs,
        ...
        if self.numerical_setup["active_physics"]["is_viscous_flux"]:
            const = 3.0 / 14.0
            kinematic_viscosity = (
                self.material_manager.get_dynamic_viscosity(temperature)
                / primes[0, ..., nhx, nhy, nhz]
            )
            if self.input_reader.levelset_type != None:
                kinematic_viscosity = kinematic_viscosity * mask_real[..., nhx_, nhy_, nhz_]
            dt = jnp.minimum(
                dt, const * (min_cell_size * min_cell_size) / jnp.max(kinematic_viscosity)
            )
        ...
        if self.numerical_setup["active_physics"]["is_heat_flux"]:
            const = 0.1
            thermal_diffusivity = (
                self.material_manager.get_thermal_conductivity(temperature)
                / primes[0, ..., nhx, nhy, nhz]
            )
            if self.input_reader.levelset_type != None:
                thermal_diffusivity = thermal_diffusivity * mask_real[..., nhx_, nhy_, nhz_]
            dt = jnp.minimum(
                dt, const * (min_cell_size * min_cell_size) / jnp.max(thermal_diffusivity)
            )
    ...
    def _feed_forward(
        self,
        primes_init: jnp.DeviceArray,
        levelset_init: jnp.DeviceArray,
        n_steps: int,
        timestep_size: float,
        t_start: float,
        output_freq: int = 1,
        ...
        if self.input_reader.levelset_type == "FLUID-FLUID":
            primes = jnp.ones(
                (
                    5,
                    2,
                    nx + 2 * nh if nx > 1 else nx,
                    ny + 2 * nh if ny > 1 else ny,
                    nz + 2 * nh if nz > 1 else nz,
                )
            )
            cons = jnp.zeros(
                (
        ...
        if self.input_reader.levelset_type == "FLUID-FLUID":
            ...
            (
                volume_fraction,
                apertures,
            ) = self.levelset_handler.compute_volume_fraction_and_apertures(levelset)
        ...
        if self.input_reader.levelset_type != None:
            ...
            out = jnp.concatenate(
                [primes_real, jnp.expand_dims(volume_fraction[nhx_, nhy_, nhz_], axis=0)], axis=0
            )
        else:
        ...
        for step in range(n_steps):
            ...
            levelset, volume_fraction, apertures = (
                levelset_quantities["levelset"],
                levelset_quantities["volume_fraction"],
                levelset_quantities["apertures"],
            )
            ...
            if current_step % output_freq == 0:
                if self.input_reader.levelset_type != None:
                    ...
                    out = jnp.concatenate(
                        [primes_real, jnp.expand_dims(volume_fraction[nhx_, nhy_, nhz_], axis=0)],
                        axis=0,
                    )
                else:
                    primes_real = primes[:, nhx, nhy, nhz]
    ...
    def feed_forward(
        self,
        batch_primes_init: jnp.DeviceArray,
        batch_levelset_init: jnp.DeviceArray,
        n_steps: int,
        timestep_size: float,
        t_start: float,
        output_freq: int = 1,
