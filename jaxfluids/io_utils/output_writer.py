...
class OutputWriter:
    ...
    def __init__(
        ...
        derivative_stencil_conservatives: SpatialDerivative,
        derivative_stencil_geometry: Union[SpatialDerivative, None],
    ) -> None:
        # GENERAL
    ...
    def write_output(
        self,
        buffer_dictionary: Dict[str, Dict[str, Union[jnp.DeviceArray, float]]],
        force_output: bool,
    ...
    def write_h5file(self, buffer_dictionary: Dict[str, Dict[str, jnp.DeviceArray]]) -> None:
        ...
        with h5py.File(os.path.join(self.save_path_domain, filename), "w") as h5file:
            ...
            h5file.create_dataset(
                name="mesh/gridX",
                data=self.unit_handler.dimensionalize(self.cell_centers[0], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/gridY",
                data=self.unit_handler.dimensionalize(self.cell_centers[1], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/gridZ",
                data=self.unit_handler.dimensionalize(self.cell_centers[2], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/gridFX",
                data=self.unit_handler.dimensionalize(self.cell_faces[0], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/gridFY",
                data=self.unit_handler.dimensionalize(self.cell_faces[1], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/gridFZ",
                data=self.unit_handler.dimensionalize(self.cell_faces[2], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/cellsizeX",
                data=self.unit_handler.dimensionalize(self.cell_sizes[0], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/cellsizeY",
                data=self.unit_handler.dimensionalize(self.cell_sizes[1], "length"),
                dtype="f8",
            )
            h5file.create_dataset(
                name="mesh/cellsizeZ",
                data=self.unit_handler.dimensionalize(self.cell_sizes[2], "length"),
                dtype="f8",
            )

            # CURRENT TIME
            h5file.create_dataset(
                name="time", data=self.unit_handler.dimensionalize(current_time, "time"), dtype="f8"
            )

            # COMPUTE TEMPERATURE
            primes = buffer_dictionary["material_fields"]["primes"]
            temperature = jnp.expand_dims(
                self.material_manager.get_temperature(primes[4], primes[0]), axis=0
            )
            material_fields = {
                "primes": jnp.vstack([primes, temperature]),
                "cons": buffer_dictionary["material_fields"]["cons"],
            }
                    ...
                    for quantity in self.output_quantities[key]:
                        if self.levelset_type == "FLUID-FLUID":
                            for i in range(2):
                                ...
                                buffer = self.unit_handler.dimensionalize(
                                    material_fields[key][
                                        self.quantity_index[key][quantity],
                                        i,
                                        self.nhx,
                                        self.nhy,
                                        self.nhz,
                            ...
                            buffer = self.unit_handler.dimensionalize(
                                material_fields[key][
                                    self.quantity_index[key][quantity], self.nhx, self.nhy, self.nhz
                                ],
                                quantity,
                            )
                            h5file.create_dataset(
                                name="/".join([key, quantity]),
                                data=buffer.T,
                                dtype="f8" if self.is_double else "f4",
                            )
            ...
            if self.levelset_type != None:
                ...
                if "levelset" in self.output_quantities.keys():
                    ...
                    levelset, volume_fraction = (
                        buffer_dictionary["levelset_quantities"]["levelset"],
                        buffer_dictionary["levelset_quantities"]["volume_fraction"],
                    )
                    ...
                    if self.levelset_type == "FLUID-FLUID":
                        (
                            ...
                            material_fields["primes"], levelset, volume_fraction
                        )
                        levelset_quantities[
                            "interface_velocity"
                        ] = self.unit_handler.dimensionalize(interface_velocity, "velocity")
                        levelset_quantities[
                            "interface_pressure"
                        ] = self.unit_handler.dimensionalize(interface_pressure, "pressure")
                        mask_real = mask_real[0]
                    elif self.levelset_type == "FLUID-SOLID-DYNAMIC":
                        interface_velocity = self.levelset_handler.compute_solid_interface_velocity(
                            current_time
                        )
                        levelset_quantities[
                            "interface_velocity"
                    ...
                    levelset_quantities.update(
                        {
                            "levelset": self.unit_handler.dimensionalize(
                                levelset[self.nhx, self.nhy, self.nhz], "length"
                            ),
                            "volume_fraction": volume_fraction[
                                ..., self.nhx_, self.nhy_, self.nhz_
                            ],
                            "mask_real": mask_real[..., self.nhx_, self.nhy_, self.nhz_],
                            "normal": normal[..., self.nhx_, self.nhy_, self.nhz_],
                        }
                    )
                    for quantity in self.output_quantities["levelset"]:
                        h5file.create_dataset(
                            name="levelset/" + quantity,
                            data=levelset_quantities[quantity].T,
                            dtype="f8" if self.is_double else "f4",
                        )
                ...
                if "real_fluid" in self.output_quantities.keys():
                    ...
                    for key in ["cons", "primes"]:
                        real_buffer = self.compute_real_buffer(
                            material_fields[key][..., self.nhx, self.nhy, self.nhz],
                            buffer_dictionary["levelset_quantities"]["volume_fraction"][
                                self.nhx_, self.nhy_, self.nhz_
                            ],
                        )
                        for quantity in [
                            quant
                            for quant in self.output_quantities["real_fluid"]
                            if quant in self.quantity_index[key].keys()
                        ]:
                            real_state = self.unit_handler.dimensionalize(
                                real_buffer[self.quantity_index[key][quantity]], quantity
                            )
                            h5file.create_dataset(
                                name="real_fluid/" + quantity,
                                data=real_state.T,
                                dtype="f8" if self.is_double else "f4",
                            )
            ...
            if "miscellaneous" in self.output_quantities.keys():
                ...
                for quantity in self.output_quantities["miscellaneous"]:
                    computed_quantity = self.compute_miscellaneous(
                        material_fields["primes"],
                        quantity,
                        buffer_dictionary["levelset_quantities"]["volume_fraction"]
                        if self.levelset_type != None
                        else None,
                    ...
                    h5file.create_dataset(
                        name="miscellaneous/" + quantity,
                        data=computed_quantity.T,
                        dtype="f8" if self.is_double else "f4",
                    )
            ...
            if self.is_mass_flow_forcing:
                ...
                h5file.create_dataset(
                    name="mass_flow_forcing/scalar_value",
                    data=buffer_dictionary["mass_flow_forcing"]["scalar_value"],
                    dtype="f8" if self.is_double else "f4",
                )
                h5file.create_dataset(
                    name="mass_flow_forcing/PID_e_int",
                    data=buffer_dictionary["mass_flow_forcing"]["PID_e_int"],
                    dtype="f8" if self.is_double else "f4",
                )
                h5file.create_dataset(
                    name="mass_flow_forcing/PID_e_new",
                    data=buffer_dictionary["mass_flow_forcing"]["PID_e_new"],
                    dtype="f8" if self.is_double else "f4",
                )
    ...
    def write_xdmffile(self, current_time: float) -> None:
        ...
        xdmf_str_start = """
            ...
            current_time,
            current_time,
            8 if self.is_double else 4,
            len(self.cell_faces[0]),
            h5file_name,
            8 if self.is_double else 4,
            len(self.cell_faces[1]),
            h5file_name,
            8 if self.is_double else 4,
            len(self.cell_faces[2]),
            h5file_name,
            len(self.cell_faces[0]),
            len(self.cell_faces[1]),
            len(self.cell_faces[2]),
            ...
            if key in self.output_quantities.keys():
                for quantity in self.output_quantities[key]:
                    ...
                    for i in range(no_phases):
                        ...
                        xdmf_quants.append(
                            self.get_xdmf(key, quantity_name, h5file_name, *self.number_of_cells)
                        )
        ...
        for key in ["real_fluid", "miscellaneous"]:
            if key in self.output_quantities.keys():
                for quantity in self.output_quantities[key]:
                    xdmf_quants.append(
                        self.get_xdmf(key, quantity, h5file_name, *self.number_of_cells)
                    )
        ...
        xdmf_str = "\n".join(
            [xdmf_preamble, xdmf_str_start] + xdmf_quants + [xdmf_str_end, xdmf_postamble]
        )
    ...
    def compute_real_buffer(
        self, buffer: jnp.DeviceArray, volume_fraction: jnp.DeviceArray
    ) -> jnp.DeviceArray:
        ...
        conservatives_real = (
            buffer[..., 0, :, :, :] * volume_fraction[0]
            + buffer[..., 1, :, :, :] * volume_fraction[1]
        )
        return conservatives_real

    @partial(jax.jit, static_argnums=(0, 2))
    def compute_miscellaneous(
        self, primes: jnp.DeviceArray, quantity: str, volume_fraction: Union[jnp.DeviceArray, None]
    ) -> jnp.DeviceArray:
        ...
        """
        if self.levelset_type == "FLUID-FLUID":
            primes_real = self.compute_real_buffer(
                primes[..., self.nhx__, self.nhy__, self.nhz__], volume_fraction
            )
        else:
            primes_real = primes
    ...
    def compute_mach_number(
        self, primes: jnp.DeviceArray, volume_fraction: Union[jnp.DeviceArray, None]
    ) -> jnp.DeviceArray:
        ...
        if self.levelset_type == "FLUID-FLUID":
            mach_number = self.compute_real_buffer(
                mach_number[..., self.nhx__, self.nhy__, self.nhz__], volume_fraction
            )[..., self.nhx_, self.nhy_, self.nhz_]
        else:
    ...
    def compute_schlieren(self, density: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        for i in range(3):
            if self.levelset_type == "FLUID-FLUID":
                schlieren.append(
                    self.derivative_stencil_geometry.derivative_xi(density, self.cell_sizes[i], i)
                    if i in self.active_axis_indices
                    else jnp.zeros(density[..., self.nhx_, self.nhy_, self.nhz_].shape)
                )
            else:
                schlieren.append(
                    self.derivative_stencil_conservatives.derivative_xi(
                        density, self.cell_sizes[i], i
                    )
                    if i in self.active_axis_indices
                    else jnp.zeros(density[..., self.nhx, self.nhy, self.nhz].shape)
                )
    ...
    def compute_vorticity(self, velocity: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        if self.levelset_type == "FLUID-FLUID":
            velocity_grad = jnp.stack(
                [
                    self.derivative_stencil_geometry.derivative_xi(velocity, self.cell_sizes[k], k)
                    if k in self.active_axis_indices
                    else jnp.zeros(velocity[..., self.nhx_, self.nhy_, self.nhz_].shape)
                    for k in range(3)
                ],
                axis=1,
            )
            ...
            velocity_grad = jnp.stack(
                [
                    self.derivative_stencil_conservatives.derivative_xi(
                        velocity, self.cell_sizes[k], k
                    )
                    if k in self.active_axis_indices
                    else jnp.zeros(velocity[..., self.nhx, self.nhy, self.nhz].shape)
                    for k in range(3)
                ],
                axis=1,
            )
        ...
        vorticity = jnp.stack([dw_dy - dv_dz, du_dz - dw_dx, dv_dx - du_dy], axis=0)
