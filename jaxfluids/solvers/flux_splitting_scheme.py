...
class FluxSplittingScheme:
    ...
    def __init__(
        ...
        self.eigendecomposition = Eigendecomposition(
            ...
            self.reconstruction_stencil._stencil_size,
            domain_information,
    ...
    def compute_fluxes_xi(
        self, primes: jnp.DeviceArray, cons: jnp.DeviceArray, axis: int, **kwargs
    ) -> jnp.DeviceArray:
        ...
        right_eigs, left_eigs, eigvals = self.eigendecomposition.eigendecomp_cons(
            stencil_prime_window, axis=axis
        )

        char = self.eigendecomposition.transformtochar(stencil_cons_window, left_eigs, axis=axis)
        char_flux = self.eigendecomposition.transformtochar(
            physical_flux_window, left_eigs, axis=axis
        )

        positive_char_flux = 0.5 * (
            char_flux + self.eigendecomposition.transformtochar(char, eigvals, axis)
        )
        negative_char_flux = 0.5 * (
            char_flux - self.eigendecomposition.transformtochar(char, eigvals, axis)
        )

        char_flux_xi_L = self.reconstruction_stencil.reconstruct_xi(
            positive_char_flux, axis, 0, dx=self.cell_sizes[axis]
        )
        char_flux_xi_R = self.reconstruction_stencil.reconstruct_xi(
            negative_char_flux, axis, 1, dx=self.cell_sizes[axis]
        )
