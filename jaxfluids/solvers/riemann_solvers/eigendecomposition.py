...
class Eigendecomposition:
    ...
    def __init__(
        ...
        self.s1_ = [
            np.arange(stencil_size).reshape(1, -1)
            + np.arange(
                self.nh - stencil_size // 2, self.number_cells[0] + self.nh - stencil_size // 2 + 1
            ).reshape(-1, 1),
            np.arange(stencil_size).reshape(1, -1)
            + np.arange(
                self.nh - stencil_size // 2, self.number_cells[1] + self.nh - stencil_size // 2 + 1
            ).reshape(-1, 1),
            np.arange(stencil_size).reshape(1, -1)
            + np.arange(
                self.nh - stencil_size // 2, self.number_cells[2] + self.nh - stencil_size // 2 + 1
            ).reshape(-1, 1),
        ]
        ...
        self.s2_ = [
            [
                jnp.s_[..., stencil_size // 2 - 1, None:None, None:None],
                jnp.s_[..., stencil_size // 2, None:None, None:None],
            ],
            [
                jnp.s_[..., None:None, stencil_size // 2 - 1, None:None],
                jnp.s_[..., None:None, stencil_size // 2, None:None],
            ],
            [
                jnp.s_[..., None:None, None:None, stencil_size // 2 - 1],
                jnp.s_[..., None:None, None:None, stencil_size // 2],
            ],
        ]
    ...
    def compute_frozen_state(self, primes: jnp.DeviceArray, axis: int) -> jnp.DeviceArray:
        ...
        if self.frozen_state == "ROE":
            ...
            enthalpy_L = self.material_manager.get_total_enthalpy(
                p=primes_L[4], rho=primes_L[0], u=primes_L[1], v=primes_L[2], w=primes_L[3]
            )
            enthalpy_R = self.material_manager.get_total_enthalpy(
                p=primes_R[4], rho=primes_R[0], u=primes_R[1], v=primes_R[2], w=primes_R[3]
            )
            ...
            squared_velocity_difference = (
                (primes_R[1] - primes_L[1]) * (primes_R[1] - primes_L[1])
                + (primes_R[2] - primes_L[2]) * (primes_R[2] - primes_L[2])
                + (primes_R[3] - primes_L[3]) * (primes_R[3] - primes_L[3])
            )

            p_over_rho_ave = (
                rho_sqrt_L * primes_L[4] / primes_L[0] + rho_sqrt_R * primes_R[4] / primes_R[0]
            ) * rho_div + 0.5 * primes_ave[0] * rho_div * rho_div * squared_velocity_difference
    ...
    def compute_roe_cons(
        self, prime_L: jnp.DeviceArray, prime_R: jnp.DeviceArray
    ) -> jnp.DeviceArray:
        ...
        """
        roe_cons = (jnp.sqrt(prime_L[0]) * prime_L + jnp.sqrt(prime_R[0]) * prime_R) / (
            jnp.sqrt(prime_L[0]) + jnp.sqrt(prime_R[0]) + self.eps
        )
        return roe_cons

    def eigendecomp_prim(
        self, primes: jnp.DeviceArray, axis: int
        ...
        (
            ...
            cc_ave,
            velocity_square,
        ) = self.compute_frozen_state(primes, axis)
        # X - DIRECTION
        if axis == 0:
    ...
    def _eigendecomp_cons(
        ...
        if axis == 0:
            ...
            left_eigs = (
                ...
                * jnp.array(
                    [
                        [
                            ek + c_ave / grueneisen_ave * primes_ave[1],
                            -primes_ave[1] - c_ave / grueneisen_ave,
                            -primes_ave[2],
                            -primes_ave[3],
                            jnp.ones(_s),
                        ],
                        [
                            2 / grueneisen_ave * cc_ave - velocity_square,
                            2 * primes_ave[1],
                            2 * primes_ave[2],
                            2 * primes_ave[3],
                            -2 * jnp.ones(_s),
                        ...
                        [
                            2 * cc_ave / grueneisen_ave * primes_ave[2],
                            jnp.zeros(_s),
                            -2 * cc_ave / grueneisen_ave,
                            jnp.zeros(_s),
                            jnp.zeros(_s),
                        ],
                        [
                            -2 * cc_ave / grueneisen_ave * primes_ave[3],
                            jnp.zeros(_s),
                            jnp.zeros(_s),
                            2 * cc_ave / grueneisen_ave,
                            jnp.zeros(_s),
        ...
        # Y - DIRECTION
            ...
            left_eigs = (
                ...
                * jnp.array(
                    [
                        [
                            ek + c_ave / grueneisen_ave * primes_ave[2],
                            -primes_ave[1],
                            -primes_ave[2] - c_ave / grueneisen_ave,
                            -primes_ave[3],
                            jnp.ones(_s),
                        ],
                        [
                            -2 * cc_ave / grueneisen_ave * primes_ave[1],
                            2 * cc_ave / grueneisen_ave,
                            jnp.zeros(_s),
                            jnp.zeros(_s),
                            jnp.zeros(_s),
        ...
        # Z - DIRECTION
            ...
            left_eigs = (
                ...
                * jnp.array(
                    [
                        [
                            ek + c_ave / grueneisen_ave * primes_ave[3],
                            -primes_ave[1],
                            -primes_ave[2],
                            -primes_ave[3] - c_ave / grueneisen_ave,
                            jnp.ones(_s),
                        ],
                        [
                            2 * cc_ave / grueneisen_ave * primes_ave[1],
        ...
        if self.flux_splitting == "CLLF":
            gamma_1 = jnp.maximum(
                jnp.abs(
                    primes[self.s2_[axis][0]][axis + 1]
                    - self.material_manager.get_speed_of_sound(
                        p=primes[self.s2_[axis][0]][4], rho=primes[self.s2_[axis][0]][0]
                    )
                ),
                jnp.abs(
                    primes[self.s2_[axis][1]][axis + 1]
                    - self.material_manager.get_speed_of_sound(
                        p=primes[self.s2_[axis][1]][4], rho=primes[self.s2_[axis][1]][0]
                    )
                ),
            )
            gamma_234 = jnp.maximum(
                jnp.abs(primes[self.s2_[axis][0]][axis + 1]),
                jnp.abs(primes[self.s2_[axis][1]][axis + 1]),
            )
            gamma_5 = jnp.maximum(
                jnp.abs(
                    primes[self.s2_[axis][0]][axis + 1]
        ...
        if axis == 0:
            right_eigs = jnp.array(
                [
                    ...
                    [
                        enthalpy_ave - primes_ave[1] * c_ave,
                        grueneisen_ave * enthalpy_ave - cc_ave,
                        -primes_ave[0] * primes_ave[2],
                        primes_ave[0] * primes_ave[3],
                        enthalpy_ave + primes_ave[1] * c_ave,
                    ],
                ]
            )
            ...
            left_eigs = jnp.array(
                [
                    [
                        ...
                        * one_cc_ave
                        * (
                            grueneisen_ave * velocity_square
                            - grueneisen_ave * enthalpy_ave
                            + (primes_ave[1] + c_ave) * c_ave
                        ),
                        0.5 * one_cc_ave * (-primes_ave[1] * grueneisen_ave - c_ave),
                        0.5 * one_cc_ave * (-primes_ave[2] * grueneisen_ave),
                        0.5 * one_cc_ave * (-primes_ave[3] * grueneisen_ave),
                        0.5 * one_cc_ave * grueneisen_ave,
                    ],
                    [
                        one_cc_ave * (enthalpy_ave - velocity_square),
                        primes_ave[1] * one_cc_ave,
                        primes_ave[2] * one_cc_ave,
                        primes_ave[3] * one_cc_ave,
                        -one_cc_ave,
                    ],
                    [
                        primes_ave[2] * one_rho_ave,
                        jnp.zeros(_s),
                        -one_rho_ave,
                        jnp.zeros(_s),
                        jnp.zeros(_s),
                ...
                [
                    [
                        ...
                        * (
                            grueneisen_ave * velocity_square
                            - grueneisen_ave * enthalpy_ave
                            + (primes_ave[2] + c_ave) * c_ave
                        ),
                        0.5 * one_cc_ave * (-primes_ave[1] * grueneisen_ave),
                        0.5 * one_cc_ave * (-primes_ave[2] * grueneisen_ave - c_ave),
                        0.5 * one_cc_ave * (-primes_ave[3] * grueneisen_ave),
                        0.5 * one_cc_ave * grueneisen_ave,
                    ],
                    [
                        -primes_ave[1] * one_rho_ave,
                        one_rho_ave,
                        jnp.zeros(_s),
                        jnp.zeros(_s),
                        jnp.zeros(_s),
                ...
                [
                    [
                        ...
                        * (
                            grueneisen_ave * velocity_square
                            - grueneisen_ave * enthalpy_ave
                            + (primes_ave[3] + c_ave) * c_ave
                        ),
                        0.5 * one_cc_ave * (-primes_ave[1] * grueneisen_ave),
                        0.5 * one_cc_ave * (-primes_ave[2] * grueneisen_ave),
                        0.5 * one_cc_ave * (-primes_ave[3] * grueneisen_ave - c_ave),
                        0.5 * one_cc_ave * grueneisen_ave,
                    ],
                    [
                        primes_ave[1] * one_rho_ave,
    ...
    def transformtochar(
        self, stencil: jnp.DeviceArray, left_eig: jnp.DeviceArray, axis: int
    ) -> jnp.DeviceArray:
    ...
    def transformtophysical(
        self, stencil: jnp.DeviceArray, right_eig: jnp.DeviceArray
    ) -> jnp.DeviceArray:
