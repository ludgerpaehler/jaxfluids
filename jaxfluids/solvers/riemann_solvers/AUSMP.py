    ...
    def solve_riemann_problem_xi(
        ...
        **kwargs,
        ...
        if self.interface_speed_of_sound == "CRITICAL":  # Eq. 40
            a_star_L = jnp.sqrt(
                2.0
                * (self.material_manager.gamma - 1.0)
                / (self.material_manager.gamma + 1.0)
                * phi_left[4]
                / phi_left[0]
            )
            ...
            a_tilde_L = a_star_L * jnp.where(
                a_star_L > jnp.abs(primes_L[axis + 1]), 1.0, 1.0 / jnp.abs(primes_L[axis + 1])
            )
            a_tilde_R = a_star_R * jnp.where(
                a_star_R > jnp.abs(primes_R[axis + 1]), 1.0, 1.0 / jnp.abs(primes_R[axis + 1])
            )
        ...
        M_plus = jnp.where(
            jnp.abs(M_l) >= 1,
            0.5 * (M_l + jnp.abs(M_l)),
            0.25 * (M_l + 1.0) * (M_l + 1.0) + self.beta * (M_l * M_l - 1.0) * (M_l * M_l - 1.0),
        )
        M_minus = jnp.where(
            jnp.abs(M_r) >= 1,
            0.5 * (M_r - jnp.abs(M_r)),
            -0.25 * (M_r - 1.0) * (M_r - 1.0) - self.beta * (M_r * M_r - 1.0) * (M_r * M_r - 1.0),
        )
        ...
        P_plus = jnp.where(
            jnp.abs(M_l) >= 1.0,
            0.5 * (1 + jnp.sign(M_l)),
            0.25 * (M_l + 1) * (M_l + 1.0) * (2.0 - M_l)
            + self.alpha * M_l * (M_l * M_l - 1.0) * (M_l * M_l - 1.0),
        )
        P_minus = jnp.where(
            jnp.abs(M_r) >= 1.0,
            0.5 * (1 - jnp.sign(M_r)),
            0.25 * (M_r - 1) * (M_r - 1.0) * (2.0 + M_r)
            - self.alpha * M_r * (M_r * M_r - 1.0) * (M_r * M_r - 1.0),
        )
        # Eq. A2
