...
class HLLCLM(RiemannSolver):
    ...
    def solve_riemann_problem_xi(
        self,
        cell_state_L: jnp.DeviceArray,
        cell_state_R: jnp.DeviceArray,
        conservative_L: jnp.DeviceArray,
        conservative_R: jnp.DeviceArray,
        axis: int,
        **kwargs,
    ) -> jnp.DeviceArray:
        ...
        speed_of_sound_left = self.material_manager.get_speed_of_sound(
            p=cell_state_L[4], rho=cell_state_L[0]
        )
        speed_of_sound_right = self.material_manager.get_speed_of_sound(
            p=cell_state_R[4], rho=cell_state_R[0]
        )
        ...
        wave_speed_simple_L, wave_speed_simple_R = self.signal_speed(
            ...
            rho_L=cell_state_L[0],
            rho_R=cell_state_R[0],
            p_L=cell_state_L[4],
            p_R=cell_state_R[4],
        ...
        """ Toro 10.73 """
        pre_factor_L = (
            (wave_speed_simple_L - cell_state_L[axis + 1])
            / (wave_speed_simple_L - wave_speed_contact)
            * cell_state_L[0]
        )
        pre_factor_R = (
            (wave_speed_simple_R - cell_state_R[axis + 1])
        ...
        u_star_L = [
            ...
            pre_factor_L
            * (
                conservative_L[4] / conservative_L[0]
                + (wave_speed_contact - cell_state_L[axis + 1])
                * (
                    wave_speed_contact
                    + cell_state_L[4]
                    / cell_state_L[0]
        ...
        u_star_R = [
            ...
            pre_factor_R
            * (
                conservative_R[4] / conservative_R[0]
                + (wave_speed_contact - cell_state_R[axis + 1])
                * (
                    wave_speed_contact
                    + cell_state_R[4]
                    / cell_state_R[0]
        ...
        Ma_local = jnp.maximum(
            jnp.abs(cell_state_L[axis + 1] / speed_of_sound_left),
            jnp.abs(cell_state_R[axis + 1] / speed_of_sound_right),
        )
        ...
        flux_star = 0.5 * (fluxes_left + fluxes_right) + 0.5 * (
            wave_speed_left * (u_star_L - conservative_L)
            + jnp.abs(wave_speed_contact) * (u_star_L - u_star_R)
            + wave_speed_right * (u_star_R - conservative_R)
        )

        """ Fleischmann et al. - 2020 - Eq. (18) """
        fluxes_xi = (
            0.5 * (1 + jnp.sign(wave_speed_simple_L)) * fluxes_left
            + 0.5 * (1 - jnp.sign(wave_speed_simple_R)) * fluxes_right
            + 0.25
            * (1 - jnp.sign(wave_speed_simple_L))
            * (1 + jnp.sign(wave_speed_simple_R))
            * flux_star
        )
