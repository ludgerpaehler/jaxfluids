...
def signal_speed_Davis_2(
    u_L: jnp.DeviceArray,
    u_R: jnp.DeviceArray,
    a_L: jnp.DeviceArray,
    a_R: jnp.DeviceArray,
    rho_L: jnp.DeviceArray,
    rho_R: jnp.DeviceArray,
    H_L: jnp.DeviceArray,
    H_R: jnp.DeviceArray,
    gamma: float,
    *args,
...
def signal_speed_Einfeldt(
    ...
    **kwargs,
    ...
    d_bar = jnp.sqrt(
        (jnp.sqrt(rho_L) * a_L * a_L + jnp.sqrt(rho_R) * a_R * a_R) * one_dens
        + eta2 * (u_R - u_L) * (u_R - u_L)
    )
    S_L = u_bar - d_bar
    S_R = u_bar + d_bar
    return S_L, S_R
