...
class SourceTermSolver:
    ...
    def __init__(
        ...
        reconstruct_stencil_duidxi: SpatialReconstruction,
        reconstruct_stencil_ui: SpatialReconstruction,
        levelset_type: str,
    ) -> None:
    ...
    def compute_gravity_forces(self, cons: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        gravity_forces = jnp.vstack(
            [jnp.zeros(gravity_energy.shape), gravity_momentum, gravity_energy]
        )
    ...
    def compute_heat_flux_xi(self, temperature: jnp.DeviceArray, axis: int) -> jnp.DeviceArray:
        ...
        temperature_grad = self.derivative_stencil_face.derivative_xi(
            temperature, self.cell_sizes[axis], axis
        )
        flux_xi = -thermal_conductivity * temperature_grad
        heat_flux_xi = jnp.vstack(
            [
                jnp.zeros(flux_xi.shape),
                jnp.zeros(flux_xi.shape),
                jnp.zeros(flux_xi.shape),
                jnp.zeros(flux_xi.shape),
                flux_xi,
    ...
    def compute_viscous_flux_xi(
        self, vels: jnp.DeviceArray, temperature: jnp.DeviceArray, axis: int
    ) -> jnp.DeviceArray:
        ...
        vel_grad = jnp.stack(
            [
                self.compute_xi_derivatives_at_xj(vels, i, axis)
                if i in self.active_axis_indices
                else jnp.zeros(self.shape_fluxes[axis])
                for i in range(3)
            ],
            axis=1,
        )
        ...
        fluxes_xj = jnp.stack(
            [jnp.zeros(tau_j[0].shape), tau_j[0], tau_j[1], tau_j[2], vel_tau_at_xj]
        )
    ...
    def compute_tau(
        self,
        vel_grad: jnp.DeviceArray,
        dynamic_viscosity: jnp.DeviceArray,
        bulk_viscosity: jnp.DeviceArray,
        axis: int,
    ) -> jnp.DeviceArray:
        ...
        tau_list = [
            mu_1 * (vel_grad[axis, 0] + vel_grad[0, axis])
            if axis in self.active_axis_indices and 0 in self.active_axis_indices
            else jnp.zeros(vel_grad.shape[2:]),
            mu_1 * (vel_grad[axis, 1] + vel_grad[1, axis])
            if axis in self.active_axis_indices and 1 in self.active_axis_indices
            else jnp.zeros(vel_grad.shape[2:]),
            mu_1 * (vel_grad[axis, 2] + vel_grad[2, axis])
    ...
    def compute_xi_derivatives_at_xj(
        self, prime: jnp.DeviceArray, axis_i: int, axis_j: int
    ) -> jnp.DeviceArray:
        ...
        if axis_i == axis_j:
            ...
            deriv_xi_at_xj = self.derivative_stencil_face.derivative_xi(
                prime, self.cell_sizes[axis_i], axis_i
            )
        else:
            # 1) Derivative at cell center 2) Interpolate to cell face
            deriv_xi_at_c = self.derivative_stencil_center.derivative_xi(
                prime, self.cell_sizes[axis_i], axis_i
            )
