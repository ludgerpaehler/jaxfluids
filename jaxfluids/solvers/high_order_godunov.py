...
class HighOrderGodunov:
    ...
    def __init__(
        ...
        reconstruction_var: str,
        is_safe_reconstruction: str,
    ) -> None:
        ...
        if self.is_safe_reconstruction:
            self.reconstruction_stencil_safe = WENO1(
        ...
        if self.reconstruction_var in ["CHAR-PRIMITIVE", "CHAR-CONSERVATIVE"]:
            ...
            self.eigendecomposition = Eigendecomposition(
                self.material_manager, self.reconstruction_stencil._stencil_size, domain_information
            )

        self.cell_sizes = domain_information.cell_sizes

    def compute_fluxes_xi(
        self,
        ...
        if self.reconstruction_var == "PRIMITIVE":
            primes_xi_L = self.reconstruction_stencil.reconstruct_xi(
                primes,
                axis,
                0,
                dx=self.cell_sizes[axis],
                ml_parameters_dict=ml_parameters_dict,
                ml_networks_dict=ml_networks_dict,
            )
            primes_xi_R = self.reconstruction_stencil.reconstruct_xi(
                primes,
                axis,
                1,
                dx=self.cell_sizes[axis],
            ...
            conservative_xi_L = get_conservatives_from_primitives(
                primes_xi_L, self.material_manager
            )
            conservative_xi_R = get_conservatives_from_primitives(
                primes_xi_R, self.material_manager
            )
            ...
            primes_xi_L = get_primitives_from_conservatives(
                conservative_xi_L, self.material_manager
            )
            primes_xi_R = get_primitives_from_conservatives(
                conservative_xi_R, self.material_manager
            )
            ...
            right_eigs, left_eigs = self.eigendecomposition.eigendecomp_prim(
                stencil_prime_window, axis
            )

            char = self.eigendecomposition.transformtochar(
                stencil_prime_window, left_eigs, axis=axis
            )

            char_xi_L = self.reconstruction_stencil.reconstruct_xi(
                char, axis, 0, dx=self.cell_sizes[axis]
            )
            char_xi_R = self.reconstruction_stencil.reconstruct_xi(
                char, axis, 1, dx=self.cell_sizes[axis]
            )
            ...
            right_eigs, left_eigs = self.eigendecomposition.eigendecomp_cons(
                stencil_prime_window, axis=axis
            )

            char = self.eigendecomposition.transformtochar(
                stencil_cons_window, left_eigs, axis=axis
            )
            char_xi_L = self.reconstruction_stencil.reconstruct_xi(
                char, axis, 0, dx=self.cell_sizes[axis]
            )
        ...
        if self.is_safe_reconstruction:
            ...
            cell_state_xi_safe_L = self.reconstruction_stencil_safe.reconstruct_xi(
                primes, axis, 0, dx=self.cell_sizes[axis]
            )
            cell_state_xi_safe_R = self.reconstruction_stencil_safe.reconstruct_xi(
                primes, axis, 1, dx=self.cell_sizes[axis]
            )
            conservative_xi_safe_L = get_conservatives_from_primitives(
                primes_xi_L, self.material_manager
            )
            conservative_xi_safe_R = get_conservatives_from_primitives(
                primes_xi_R, self.material_manager
            )
