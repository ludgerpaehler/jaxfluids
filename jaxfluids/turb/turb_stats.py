...
from jaxfluids.turb.turb_utils import (calculate_dilatation,
                                       calculate_sheartensor,
                                       calculate_vorticity,
...
def get_energy_spectrum(vel):
    ...
    uu = (
        ...
        * 0.5
        * (
            jnp.abs(vel_hat[0] * vel_hat[0])
            + jnp.abs(vel_hat[1] * vel_hat[1])
            + jnp.abs(vel_hat[2] * vel_hat[2])
        )
    )
...
def get_stats_hit(
    ...
    Ma_t = (
        np.sqrt(np.mean(velocityX**2 + velocityY**2 + velocityZ**2)) / speed_of_sound_mean_1
    )
...
def get_quick_stats_hit(
    rho, velocityX, velocityY, velocityZ, pressure, mu: float, gamma: float = 1.4
):
    print("\n\n********************\nTURB STATS HIT")
    U = np.stack([velocityX, velocityY, velocityZ])
    ...
    print(
        "Taylor microscale = %4.3e \nLongitudinal Taylor microscale = %4.3e \nLateral Taylor microscale = %4.3e"
        % (lambda_, lambda_g, lambda_f)
    )
...
def get_energy_spectrum_np_OLD(vel):
    ...
    N = Nx
    Nf = N // 2 + 1
    k = np.fft.fftfreq(N, 1.0 / N)  # for y and z direction
    kx = k[:Nf].copy()
    kx[-1] *= -1
    ...
    energy_spec = (
        0.5
        * np.mean(
            vel_hat_2[0] * np.conj(vel_hat_2[0])
            + vel_hat_2[1] * np.conj(vel_hat_2[1])
            + vel_hat_2[2] * np.conj(vel_hat_2[2])
        )
        / N**3
    )
    ...
    k_mag_2 = np.sqrt(
        k_field_2[0] * k_field_2[0] + k_field_2[1] * k_field_2[1] + k_field_2[2] * k_field_2[2]
    )
    ...
    uu_2 = 0.5 * (
        np.abs(vel_hat_2[0] * vel_hat_2[0])
        + jnp.abs(vel_hat_2[1] * vel_hat_2[1])
        + jnp.abs(vel_hat_2[2] * vel_hat_2[2])
    )
