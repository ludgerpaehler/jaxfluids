...
def calculate_vorticity_spectral(
    velocity_hat: np.array, k_field: np.array, dtype: np.dtype = np.float64
) -> np.array:
    ...
    omega[0] = jnp.fft.irfftn(
        1j * (k_field[1] * velocity_hat[2] - k_field[2] * velocity_hat[1]), axes=(2, 1, 0)
    )
    omega[1] = jnp.fft.irfftn(
        1j * (k_field[2] * velocity_hat[0] - k_field[0] * velocity_hat[2]), axes=(2, 1, 0)
    )
    omega[2] = jnp.fft.irfftn(
        1j * (k_field[0] * velocity_hat[1] - k_field[1] * velocity_hat[0]), axes=(2, 1, 0)
    )
...
def calculate_dilatation_spectral(
    ...
    dilatation_spectral = 1j * (
        k_field[0] * velocity_hat[0] + k_field[1] * velocity_hat[1] + k_field[2] * velocity_hat[2]
    )
