...
class TurbStatsManager:
    ...
    def __init__(
        self, domain_information: DomainInformation, material_manager: MaterialManager = None
    ) -> None:
        ...
        assert (
            self.Nx == self.Ny and self.Ny == self.Nz
        ), "The present implementation only works for cubic domains."
    ...
    def initialize(self) -> None:
        ...
        self.k2_field = (
            self.k_field[0] * self.k_field[0]
            + self.k_field[1] * self.k_field[1]
            + self.k_field[2] * self.k_field[2]
        )
        ...
        self.fact = (
            2 * (self.k_field[0] > 0) * (self.k_field[0] < self.Nx // 2)
            + 1 * (self.k_field[0] == 0)
            + 1 * (self.k_field[0] == self.Nx // 2)
        )
    ...
    def _get_real_wavenumber_grid(self, N: int) -> Tuple[jnp.DeviceArray, jnp.DeviceArray]:
        ...
        Nf = N // 2 + 1
        k = jnp.fft.fftfreq(N, 1.0 / N)  # for y and z direction
        kx = k[:Nf]
        kx = kx.at[-1].mul(-1)
    ...
    def energy_spectrum_spectral(self, velocity_hat: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        uu = (
            ...
            * 0.5
            * (
                jnp.abs(velocity_hat[0] * velocity_hat[0])
                + jnp.abs(velocity_hat[1] * velocity_hat[1])
                + jnp.abs(velocity_hat[2] * velocity_hat[2])
            )
        )
    ...
    def hit_statistics(self, primes: jnp.DeviceArray) -> Dict:
        ...
        u_rms = u_prime = jnp.sqrt(
            jnp.mean(velocity[0] ** 2 + velocity[1] ** 2 + velocity[2] ** 2) / 3
        )
        q_rms = jnp.sqrt(jnp.mean(velocity[0] ** 2 + velocity[1] ** 2 + velocity[2] ** 2))

        Ma_t = (
            jnp.sqrt(jnp.mean(velocity[0] ** 2 + velocity[1] ** 2 + velocity[2] ** 2))
            / speed_of_sound_mean_1
        )
        Ma_rms = jnp.sqrt(
            jnp.mean((velocity[0] ** 2 + velocity[1] ** 2 + velocity[2] ** 2) / speed_of_sound)
        )
        ...
        vorticity_rms = jnp.sqrt(
            jnp.mean(vorticity[0] ** 2 + vorticity[1] ** 2 + vorticity[2] ** 2) / 3
        )
    ...
    def calculate_vorticity_spectral(self, velocity_hat: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        omega_0 = jnp.fft.irfftn(
            1j * (self.k_field[1] * velocity_hat[2] - self.k_field[2] * velocity_hat[1]),
            axes=(2, 1, 0),
        )
        omega_1 = jnp.fft.irfftn(
            1j * (self.k_field[2] * velocity_hat[0] - self.k_field[0] * velocity_hat[2]),
            axes=(2, 1, 0),
        )
        omega_2 = jnp.fft.irfftn(
            1j * (self.k_field[0] * velocity_hat[1] - self.k_field[1] * velocity_hat[0]),
            axes=(2, 1, 0),
        )
    ...
    def calculate_sheartensor_spectral(self, velocity_hat: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        for ii in range(3):
            for jj in range(3):
                duidj[ii].append(
                    jnp.fft.irfftn(1j * self.k_field[jj] * velocity_hat[ii], axes=(2, 1, 0))
                )
    ...
    def calculate_dilatation_spectral(self, velocity_hat: jnp.DeviceArray) -> jnp.DeviceArray:
        ...
        dilatation_spectral = 1j * (
            self.k_field[0] * velocity_hat[0]
            + self.k_field[1] * velocity_hat[1]
            + self.k_field[2] * velocity_hat[2]
        )
