...
class TurbInitManager:
    ...
    def __init__(
        ...
        self.turbulent_random_seed = (
            self.input_reader.turb_init_params["seed"]
            if "seed" in self.input_reader.turb_init_params.keys()
            else 0
        )
...
def turb_init_TGV(
    X: np.ndarray, gamma: float, Ma: float, rho_ref: float, V_ref: float, L: float
) -> np.ndarray:
    ...
    pressure = (
        ...
        * (
            ...
            + 1
            / 16.0
            * ((np.cos(2 * X[0] / L) + np.cos(2 * X[1] / L)) * (np.cos(2 * X[2] / L) + 2))
        )
    )
    return density, velocityX, velocityY, velocityZ, pressure
...
def _turb_init_spyropoulos_v2(
    ...
    print(
        "Reference Values: p_ref = %3.2e, rho_ref = %3.2e, T_ref = %3.2e, c_ref = %3.2e"
        % (p_ref, rho_ref, T_ref, c_ref)
    )
    ...
    print(
        "q_s = %4.3e, q_d = %4.3e, psi = (q_d/q)^2 = %4.3e"
        % (U1_rms, U2_rms, (U2_rms / U_rms) ** 2)
    )
...
def turb_init_ristorcelli(
    ...
    if ic_type == "IC1":
        ...
        velocity, pressure, density = get_dilatational_field(
            velocity, ma_target, T_ref, rho_ref, gamma
        )
        pressure = np.abs(pressure)
...
def get_solenoidal_field(U: np.ndarray) -> np.ndarray:
    ...
    print(
        ...
        np.sum(
            np.abs(
                k_field[0] * U_sol_hat[0] + k_field[1] * U_sol_hat[1] + k_field[2] * U_sol_hat[2]
            )
        ),
    )
...
def get_dilatational_field(
    U_field: np.array, Ma: float, T_ref: float, rho_ref: float, gamma: float
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
