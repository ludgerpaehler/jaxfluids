...
class Initializer:
    ...
    def __init__(self, input_reader: InputReader) -> None:
        ...
        config.update(
            "jax_enable_x64",
            self.input_reader.numerical_setup["output"]["is_double_precision_compute"],
        )

        self.eps = (
            jnp.finfo(jnp.float64).eps
            if self.input_reader.numerical_setup["output"]["is_double_precision_compute"]
        ...
        if self.input_reader.is_turb_init:
            self.turb_init_manager = TurbInitManager(
                self.input_reader, self.domain_information, self.material_manager
            )

    def initialization(
        self, user_prime_init: Union[np.ndarray, jnp.DeviceArray] = None
    ) -> Dict[str, Dict[str, Union[jnp.DeviceArray, float]]]:
        ...
        if self.input_reader.restart_flag:
            ...
            assert (
                self.unit_handler.non_dimensionalize(self.input_reader.end_time, "time")
                > current_time
            ), "The end time %.4e is not larger than the restart time %.4e" % (
                self.input_reader.end_time,
                current_time,
            )
            ...
            if self.input_reader.levelset_type == "FLUID-FLUID":
                ...
                density_0 = self.unit_handler.non_dimensionalize(
                    h5file["primes/density_0"][:].T, "density"
                )
                density_1 = self.unit_handler.non_dimensionalize(
                    h5file["primes/density_1"][:].T, "density"
                )
                density = jnp.stack([density_0, density_1], axis=0)
                pressure_0 = self.unit_handler.non_dimensionalize(
                    h5file["primes/pressure_0"][:].T, "pressure"
                )
                pressure_1 = self.unit_handler.non_dimensionalize(
                    h5file["primes/pressure_1"][:].T, "pressure"
                )
                pressure = jnp.stack([pressure_0, pressure_1], axis=0)
                if "velocity_0" in h5file["primes"].keys():
                    velocity_0 = self.unit_handler.non_dimensionalize(
                        h5file["primes/velocity_0"][:].T, "velocity"
                    )
                    velocity_1 = self.unit_handler.non_dimensionalize(
                        h5file["primes/velocity_1"][:].T, "velocity"
                    )
                else:
                    velocityX_0 = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityX_0"][:].T, "velocity"
                        )
                        if 0 in active_axis_indices
                        else jnp.zeros_like(density_0)
                    )
                    velocityX_1 = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityX_1"][:].T, "velocity"
                        )
                        if 0 in active_axis_indices
                        else jnp.zeros_like(density_0)
                    )
                    ...
                    velocityY_0 = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityY_0"][:].T, "velocity"
                        )
                        if 1 in active_axis_indices
                        else jnp.zeros_like(density_0)
                    )
                    velocityY_1 = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityY_1"][:].T, "velocity"
                        )
                        if 1 in active_axis_indices
                        else jnp.zeros_like(density_0)
                    )
                    ...
                    velocityZ_0 = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityZ_0"][:].T, "velocity"
                        )
                        if 2 in active_axis_indices
                        else jnp.zeros_like(density_0)
                    )
                    velocityZ_1 = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityZ_1"][:].T, "velocity"
                        )
                        if 2 in active_axis_indices
                        else jnp.zeros_like(density_0)
                    )
                ...
                primes_init = jnp.vstack(
                    [jnp.expand_dims(density, axis=0), velocity, jnp.expand_dims(pressure, axis=0)]
                )
                primes = primes.at[..., nhx, nhy, nhz].set(primes_init)
            # ELSE
            else:
                density = self.unit_handler.non_dimensionalize(
                    h5file["primes/density"][:].T, "density"
                )
                pressure = self.unit_handler.non_dimensionalize(
                    h5file["primes/pressure"][:].T, "pressure"
                )
                if "velocity" in h5file["primes"].keys():
                    velocity = self.unit_handler.non_dimensionalize(
                        h5file["primes/velocity"][:].T, "velocity"
                    )
                else:
                    velocityX = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityX"][:].T, "velocity"
                        )
                        if 0 in active_axis_indices
                    ...
                    velocityY = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityY"][:].T, "velocity"
                        )
                        if 1 in active_axis_indices
                        else jnp.zeros_like(density)
                    )
                    velocityZ = (
                        self.unit_handler.non_dimensionalize(
                            h5file["primes/velocityZ"][:].T, "velocity"
                        )
                        if 2 in active_axis_indices
                        else jnp.zeros_like(density)
                    )
            ...
            if self.input_reader.levelset_type == "FLUID-FLUID":
                assert (
                    ...
                    ny,
                    nz,
                ) == density.shape, "Resolution in case setup %s is not same as in restart file %s" % (
                    (nx, ny, nz),
                    density.shape,
                )
            else:
            ...
            if self.input_reader.levelset_type != None:
                ...
                levelset_init = self.unit_handler.non_dimensionalize(
                    h5file["levelset/levelset"][:].T, "length"
                )
        ...
        # FILL BUFFERS FROM TURBULENT INITIAL CONDITIONS
            ...
            (
                ...
                velocityZ,
                pressure,
            ) = self.turb_init_manager.get_turbulent_initial_condition()
        ...
        # USER SPECIFIED INIT PRIME BUFFER
            ...
            user_prime_init[1:4] = self.unit_handler.non_dimensionalize(
                user_prime_init[1:4], "velocity"
            )
            user_prime_init[4] = self.unit_handler.non_dimensionalize(
                user_prime_init[4], "pressure"
            )
        ...
        # FILL BUFFERS USING THE INITIAL CONDITION FOR THE PRIMITIVE VARIABLES SPECIFIED IN THE CASE SETUP
            ...
            mesh_grid = [
                jnp.meshgrid(*self.domain_information.cell_centers, indexing="ij")[i]
                for i in active_axis_indices
            ]
            ...
            for i in range(fluid_phases):
                initial_condition = (
                    self.input_reader.initial_condition[["positive", "negative"][i]]
                    if self.input_reader.levelset_type == "FLUID-FLUID"
                    else self.input_reader.initial_condition
                )
                primes_init_fluid = []
            ...
            if self.input_reader.levelset_type != None:
                ...
                cons, primes, _ = self.levelset_handler.extend_primes(
                    cons, primes, levelset, volume_fraction, current_time
                )
        ...
        # MASS FLOW FORCING
        if (
            self.input_reader.restart_flag
            and self.numerical_setup["active_forcings"]["is_mass_flow_forcing"]
        ):
