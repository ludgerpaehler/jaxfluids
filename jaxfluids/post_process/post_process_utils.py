...
def load_data(
    path: str, quantities: List, start: int = 0, end: int = None, N: int = 1
...
def create_contourplot(
    ...
    # CREATE SUBPLOTS
    fig, axes, quadmesh_list, pciset_list = create_figure_2D(
        data, levelset, nrows_ncols, titles, X1, X2, minmax, index_time
    )
    ...
    ani = FuncAnimation(
        fig,
        update_2D,
        frames=data[0].shape[0],
        fargs=(X1, X2, data, levelset, axes, quadmesh_list, pciset_list),
        interval=interval,
        blit=True,
        repeat=True,
    )
    ...
    for z, ax, quadmesh, pciset in zip(data, axes, quadmesh_list, pciset_list):
        if type(levelset) != type(None) and type(pciset[0]) != type(None):
            for tp in pciset[0].collections:
                tp.remove()
            pciset[0] = ax.contour(
                X, Y, np.squeeze(levelset[i, :, :]), levels=[0.0], colors="black", linewidths=4
            )
    ...
    for i, quantity in enumerate(titles):
        if quantity == "schlieren":
            ...
            kwargs.append(
                dict(
                    cmap="binary",
                    norm=matplotlib.colors.LogNorm(vmin=minmax[i][0], vmax=minmax[i][1]),
                )
            )
        else:
...
def generate_png(
    ...
    for quant in quantities:
        if quant == "schlieren":
            kwargs.append(
                {
                    ...
                    "norm": matplotlib.colors.LogNorm(
                        vmin=np.min(data_dictionary[quant]), vmax=np.max(data_dictionary[quant])
                    ),
                }
            )
        else:
            kwargs.append(
                {
                    "cmap": "seismic",
                    "vmin": np.min(data_dictionary[quant]),
                    "vmax": np.max(data_dictionary[quant]),
                }
            )
    ...
    assert np.prod(nrows_ncols) == len(
        quantities
    ...
    for i in range(no_timesnapshots):
        ...
        for ax, quantity, kw in zip(axes, data_dictionary, kwargs):
            pcolormesh = ax.pcolormesh(
                X, Y, data_dictionary[quantity][i, :, :, 0], **kw, shading="auto"
            )
            if type(levelset) != type(None):
            ...
            ax.tick_params(
                labelsize=fontsize, left=False, bottom=False, labelleft=False, labelbottom=False
            )
            ax.set_aspect("equal")
