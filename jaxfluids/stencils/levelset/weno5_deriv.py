...
class WENO5DERIV(SpatialDerivative):
        ...
        self._slices = [
            [
                [
                    ...
                    jnp.s_[
                        ...,
                        jnp.s_[self.n + 1 + j : -self.n + 2 + j]
                        if -self.n + 2 + j != 0
                        else jnp.s_[self.n + 1 + j : None],
                        self.nhy,
                        self.nhz,
                    ],
                ],
    ...
    def derivative_xi(
        self, levelset: jnp.DeviceArray, dx: int, i: int, j: int, *args
    ) -> jnp.DeviceArray:
        levelset = self.derivative_stencil.derivative_xi(levelset, dx, i, j=self.sided_deriv_upwind)

        s1_ = self._slices[j][i]

        beta_0 = 13.0 / 12.0 * (levelset[s1_[0]] - 2 * levelset[s1_[1]] + levelset[s1_[2]]) * (
            levelset[s1_[0]] - 2 * levelset[s1_[1]] + levelset[s1_[2]]
        ) + 1.0 / 4.0 * (levelset[s1_[0]] - 4 * levelset[s1_[1]] + 3 * levelset[s1_[2]]) * (
            levelset[s1_[0]] - 4 * levelset[s1_[1]] + 3 * levelset[s1_[2]]
        )
        beta_1 = 13.0 / 12.0 * (levelset[s1_[1]] - 2 * levelset[s1_[2]] + levelset[s1_[3]]) * (
            levelset[s1_[1]] - 2 * levelset[s1_[2]] + levelset[s1_[3]]
        ) + 1.0 / 4.0 * (levelset[s1_[1]] - levelset[s1_[3]]) * (
            levelset[s1_[1]] - levelset[s1_[3]]
        )
        beta_2 = 13.0 / 12.0 * (levelset[s1_[2]] - 2 * levelset[s1_[3]] + levelset[s1_[4]]) * (
            levelset[s1_[2]] - 2 * levelset[s1_[3]] + levelset[s1_[4]]
        ) + 1.0 / 4.0 * (3 * levelset[s1_[2]] - 4 * levelset[s1_[3]] + levelset[s1_[4]]) * (
            3 * levelset[s1_[2]] - 4 * levelset[s1_[3]] + levelset[s1_[4]]
        )
        ...
        p_0 = (
            self.cr_[j][0][0] * levelset[s1_[0]]
            + self.cr_[j][0][1] * levelset[s1_[1]]
            + self.cr_[j][0][2] * levelset[s1_[2]]
        )
        p_1 = (
            self.cr_[j][1][0] * levelset[s1_[1]]
            + self.cr_[j][1][1] * levelset[s1_[2]]
            + self.cr_[j][1][2] * levelset[s1_[3]]
        )
        p_2 = (
            self.cr_[j][2][0] * levelset[s1_[2]]
            + self.cr_[j][2][1] * levelset[s1_[3]]
            + self.cr_[j][2][2] * levelset[s1_[4]]
        )
