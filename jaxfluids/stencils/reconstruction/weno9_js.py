...
class WENO9(SpatialReconstruction):
    ...
    def reconstruct_xi(
        ...
        beta_0 = (
            ...
            * (
                482963 * buffer[s1_[1]]
                - 1704396 * buffer[s1_[2]]
                + 1358458 * buffer[s1_[3]]
                - 411487 * buffer[s1_[4]]
            )
            + buffer[s1_[2]]
            * (1521393 * buffer[s1_[2]] - 2462076 * buffer[s1_[3]] + 758823 * buffer[s1_[4]])
        ...
        beta_1 = (
            ...
            * (
            ...
            + buffer[s1_[3]]
            * (406293 * buffer[s1_[3]] - 611976 * buffer[s1_[4]] + 165153 * buffer[s1_[5]])
        ...
        beta_2 = (
            ...
            * (
            ...
            + buffer[s1_[4]]
            * (231153 * buffer[s1_[4]] - 299076 * buffer[s1_[5]] + 67923 * buffer[s1_[6]])
        ...
        beta_3 = (
            ...
            * (
            ...
            + buffer[s1_[5]]
            * (406293 * buffer[s1_[5]] - 464976 * buffer[s1_[6]] + 99213 * buffer[s1_[7]])
        ...
        beta_4 = (
            ...
            * (
            ...
            + buffer[s1_[5]]
            * (
                1020563 * buffer[s1_[5]]
                - 2462076 * buffer[s1_[6]]
                + 1358458 * buffer[s1_[7]]
                - 288007 * buffer[s1_[8]]
            )
            + buffer[s1_[6]]
            * (1521393 * buffer[s1_[6]] - 1704396 * buffer[s1_[7]] + 364863 * buffer[s1_[8]])
        ...
        p_0 = (
            ...
            + self.cr_[j][0][3] * buffer[s1_[3]]
            + self.cr_[j][0][4] * buffer[s1_[4]]
        )
        p_1 = (
            self.cr_[j][1][0] * buffer[s1_[1]]
            + self.cr_[j][1][1] * buffer[s1_[2]]
            + self.cr_[j][1][2] * buffer[s1_[3]]
            + self.cr_[j][1][3] * buffer[s1_[4]]
            + self.cr_[j][1][4] * buffer[s1_[5]]
        )
        p_2 = (
            self.cr_[j][2][0] * buffer[s1_[2]]
            + self.cr_[j][2][1] * buffer[s1_[3]]
            + self.cr_[j][2][2] * buffer[s1_[4]]
            + self.cr_[j][2][3] * buffer[s1_[5]]
            + self.cr_[j][2][4] * buffer[s1_[6]]
        )
        p_3 = (
            self.cr_[j][3][0] * buffer[s1_[3]]
            + self.cr_[j][3][1] * buffer[s1_[4]]
            + self.cr_[j][3][2] * buffer[s1_[5]]
            + self.cr_[j][3][3] * buffer[s1_[6]]
            + self.cr_[j][3][4] * buffer[s1_[7]]
        )
        p_4 = (
            self.cr_[j][4][0] * buffer[s1_[4]]
            + self.cr_[j][4][1] * buffer[s1_[5]]
            + self.cr_[j][4][2] * buffer[s1_[6]]
            + self.cr_[j][4][3] * buffer[s1_[7]]
            + self.cr_[j][4][4] * buffer[s1_[8]]
        )

        cell_state_xi_j = (
            omega_0 * p_0 + omega_1 * p_1 + omega_2 * p_2 + omega_3 * p_3 + omega_4 * p_4
        )
