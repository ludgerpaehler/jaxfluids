...
class WENO6CUM1(SpatialReconstruction):
    ...
    def reconstruct_xi(
        ...
        alpha_0 = self.dr_[j][0] * jnp.power(
            (self.Cq_ + tau_6 / (beta_0 + self.eps * dx2)), self.q_
        )
        alpha_1 = self.dr_[j][1] * jnp.power(
            (self.Cq_ + tau_6 / (beta_1 + self.eps * dx2)), self.q_
        )
        alpha_2 = self.dr_[j][2] * jnp.power(
            (self.Cq_ + tau_6 / (beta_2 + self.eps * dx2)), self.q_
        )
        alpha_3 = self.dr_[j][3] * jnp.power(
            (self.Cq_ + tau_6 / (beta_3 + self.eps * dx2)), self.q_
        )
