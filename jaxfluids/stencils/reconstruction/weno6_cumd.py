...
class WENO6CUMD(SpatialReconstruction):
    ...
    def reconstruct_xi(
        self, buffer: jnp.DeviceArray, axis: int, j: int, dx: float, **kwargs
    ) -> jnp.DeviceArray:
        ...
        beta_3 = (
            ...
            * (
                ...
                + buffer[s1_[0]]
                * (
                    -2380800 * buffer[s1_[1]]
                    + 4086352 * buffer[s1_[2]]
                    - 3462252 * buffer[s1_[3]]
                    + 1458762 * buffer[s1_[4]]
                    - 245620 * buffer[s1_[5]]
                )
                + buffer[s1_[1]]
                * (
                    5653317 * buffer[s1_[1]]
                    - 20427884 * buffer[s1_[2]]
                    + 17905032 * buffer[s1_[3]]
                    - 7727988 * buffer[s1_[4]]
                    + 1325006 * buffer[s1_[5]]
                )
                + buffer[s1_[2]]
                * (
                    19510972 * buffer[s1_[2]]
                    - 35817664 * buffer[s1_[3]]
                    + 15929912 * buffer[s1_[4]]
                    - 2792660 * buffer[s1_[5]]
                )
                + buffer[s1_[3]]
                * (17195652 * buffer[s1_[3]] - 15880404 * buffer[s1_[4]] + 2863984 * buffer[s1_[5]])
        ...
        alpha_0 = self.dr_[j][0] * jnp.power(
            (self.C_ + tau_6 / (beta_0 + self.eps) + dx ** (1 / 2) * beta_0 / (tau_6 + self.eps)),
            self.q_,
        )
        alpha_1 = self.dr_[j][1] * jnp.power(
            (self.C_ + tau_6 / (beta_1 + self.eps) + dx ** (1 / 2) * beta_1 / (tau_6 + self.eps)),
            self.q_,
        )
        alpha_2 = self.dr_[j][2] * jnp.power(
            (self.C_ + tau_6 / (beta_2 + self.eps) + dx ** (1 / 2) * beta_2 / (tau_6 + self.eps)),
            self.q_,
        )
        alpha_3 = self.dr_[j][3] * jnp.power(
            (self.C_ + tau_6 / (beta_3 + self.eps) + dx ** (1 / 2) * beta_3 / (tau_6 + self.eps)),
            self.q_,
        )
