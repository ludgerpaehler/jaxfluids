...
class Forcing:
    ...
    def __init__(
        ...
        levelset_handler: Union[LevelsetHandler, None],
        levelset_type: str,
        is_mass_flow_forcing: bool,
        is_temperature_forcing: bool,
        is_turb_hit_forcing: bool,
        mass_flow_target: Union[float, types.LambdaType],
        flow_direction: str,
        temperature_target: Union[float, types.LambdaType],
    ) -> None:
        # BOOLS FORCINGS
        ...
        self.inactive_axis_indices = [
            {"x": 0, "y": 1, "z": 2}[axis] for axis in domain_information.inactive_axis
        ]
    ...
    def compute_forcings(
        ...
        levelset: Union[jnp.DeviceArray, None],
        volume_fraction: Union[jnp.DeviceArray, None],
        current_time: float,
        timestep_size: float,
        PID_e_new: float,
        PID_e_int: float,
        logger: Logger,
        primes_dash: Union[jnp.DeviceArray, None] = None,
        **kwargs,
    ) -> Dict:
        ...
        if self.is_mass_flow_forcing:
            (
                ...
                PID_e_new,
                PID_e_int,
            ) = self.compute_mass_flow_forcing(
                cons, primes, volume_fraction, current_time, timestep_size, PID_e_new, PID_e_int
            )
        ...
        if self.is_temperature_forcing:
            temperature_forcing, temperature_error = self.compute_temperature_forcing(
                primes, levelset, volume_fraction, current_time, timestep_size
            )
            forcings_dictionary.update(
                {
                    "temperature": {
        ...
        if self.is_turb_hit_forcing:
            ...
            forcings_dictionary.update({"turbulence": {"force": turb_hit_forcing}})
    ...
    def compute_temperature_forcing(
        ...
        mesh_grid = [
            jnp.meshgrid(*self.cell_centers, indexing="ij")[i] for i in self.active_axis_indices
        ]
        ...
        R, gamma, rho = (
            self.material_manager.R,
            self.material_manager.gamma,
            primes[0, ..., self.nhx, self.nhy, self.nhz],
        )
        temperature_error = (
            (temperature_target - temperature) * mask_real[..., self.nhx_, self.nhy_, self.nhz_]
            if self.levelset_type != None
            else temperature_target - temperature
        )
    ...
    def compute_mass_flow_forcing(
        ...
        PID_e_new: float,
        PID_e_int: float,
    ) -> Tuple[jnp.DeviceArray, float, float, float, float]:
        ...
        momentum = (
            cons[self.index, ..., self.nhx, self.nhy, self.nhz]
            * volume_fraction[..., self.nhx_, self.nhy_, self.nhz_]
            if self.levelset_type != None
            else cons[self.index, ..., self.nhx, self.nhy, self.nhz]
        )
        mass_flow_current = jnp.mean(jnp.sum(self.dA * momentum, axis=self.int_ax), axis=-1)
        mass_flow_current = (
            jnp.sum(mass_flow_current) if self.levelset_type == "FLUID-FLUID" else mass_flow_current
        )

        # COMPUTE MASS FLOW FORCING
        mass_flow_forcing_scalar, PID_e_new, PID_e_int = self.PID_mass_flow_forcing.compute_output(
            mass_flow_current, mass_flow_target, timestep_size, PID_e_new, PID_e_int
        )
        ...
        body_force_momentum = jnp.einsum(
            "ij..., jk...->ik...", mass_flow_forcing.reshape(3, 1), jnp.ones(density.shape)
        )
        body_force_energy = jnp.einsum("ij..., jk...->ik...", mass_flow_forcing.reshape(1, 3), vels)

        body_force = jnp.vstack(
            [jnp.zeros(body_force_energy.shape), body_force_momentum, body_force_energy]
        )
    ...
    def compute_turb_hit_forcing(
        self, primes: jnp.DeviceArray, primes_dash: jnp.DeviceArray, timestep: float
    ) -> jnp.DeviceArray:
        ...
        s_1, s_2, s_3 = self.calculate_velocity_forcing_vector(
            primes[1:4], primes_dash[1:4], eta_s, timestep
        )
        s_4 = (
            primes[1] * s_1
            + primes[2] * s_2
            + primes[3] * s_3
            + (self.temperature_target - Tbar)
            * self.material_manager.R
            / (self.material_manager.gamma - 1)
        )
        force = [s_0, s_1, s_2, s_3, s_4]
        return primes[0] * jnp.stack(force)

    def calculate_velocity_forcing_vector(
        self, vels: jnp.DeviceArray, vels_dash: jnp.DeviceArray, eta_s: int, timestep: float
    ) -> jnp.DeviceArray:
        ...
        div_u = (
            self.k_field[0] * vels_hat[0]
            + self.k_field[1] * vels_hat[1]
            + self.k_field[2] * vels_hat[2]
        )
