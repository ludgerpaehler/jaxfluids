...
class SpaceSolver:
    ...
    def __init__(
        ...
        gravity: jnp.DeviceArray,
        levelset_type: Tuple[None, str],
        ...
        self.source_term_solver = SourceTermSolver(
            ...
            derivative_stencil_center=DICT_FIRST_DERIVATIVE_CENTER[
                ...
                offset=2
                if numerical_setup["conservatives"]["dissipative_fluxes"]["reconstruction_stencil"]
                == "R4"
                else 1,
            )
            ...
            derivative_stencil_face=DICT_DERIVATIVE_FACE[
                numerical_setup["conservatives"]["dissipative_fluxes"]["derivative_stencil_face"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
                offset=0,
            ...
            if numerical_setup["active_physics"]["is_viscous_flux"]
            or numerical_setup["active_physics"]["is_heat_flux"]
            else None,
            reconstruct_stencil_duidxi=DICT_CENTRAL_RECONSTRUCTION[
                numerical_setup["conservatives"]["dissipative_fluxes"]["reconstruction_stencil"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
                offset=domain_information.nh_conservatives - 2
                if numerical_setup["conservatives"]["dissipative_fluxes"]["reconstruction_stencil"]
                == "R4"
                else domain_information.nh_conservatives - 1,
            )
            if numerical_setup["active_physics"]["is_viscous_flux"]
            else None,
            reconstruct_stencil_ui=DICT_CENTRAL_RECONSTRUCTION[
                numerical_setup["conservatives"]["dissipative_fluxes"]["reconstruction_stencil"]
            ](
                nh=domain_information.nh_conservatives,
                inactive_axis=domain_information.inactive_axis,
    ...
    def compute_rhs(
        ...
        current_time: float,
        levelset: jnp.DeviceArray = None,
        volume_fraction: jnp.DeviceArray = None,
        apertures: List = None,
        forcings_dictionary: Union[Dict, None] = None,
        ml_parameters_dict: Union[Dict, None] = None,
        ml_networks_dict: Union[Dict, None] = None,
    ) -> Tuple[jnp.DeviceArray, Union[jnp.DeviceArray, None], Union[float, None]]:
        ...
        if self.levelset_type == "FLUID-FLUID":
            (
                ...
                primes, levelset, volume_fraction
            )
        elif self.levelset_type == "FLUID-SOLID-DYNAMIC":
            interface_velocity, interface_pressure, residual_interface = (
                self.levelset_handler.compute_solid_interface_velocity(current_time),
                None,
                None,
            )
        else:
        ...
        for axis in self.active_axis_indices:
            ...
            if self.is_convective_flux:
                flux_xi += self.flux_computer.compute_convective_flux_xi(
                    primes, cons, axis, ml_parameters_dict, ml_networks_dict
                )

            # VISCOUS CONTRIBUTION
            if self.is_viscous_flux:
                flux_xi -= self.source_term_solver.compute_viscous_flux_xi(
                    primes[1:4], temperature, axis
                )
            ...
            if self.is_convective_flux or self.is_viscous_flux or self.is_heat_flux:
                rhs_cons += self.one_cell_size[axis] * (
            ...
            if self.levelset_type in ["FLUID-FLUID", "FLUID-SOLID-DYNAMIC"]:
                rhs_contribution_levelset = self.levelset_handler.compute_levelset_advection_rhs(
